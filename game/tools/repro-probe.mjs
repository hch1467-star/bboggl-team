 ...
 *
 * ── ⚠️ **이 프로브는 혼자 돌려야 합니다** ──────────────────────────
 * 다른 프로브와 **동시에** 돌리면 빨간불이 납니다. 게임이 망가져서가
 * 아니라, 브라우저 두 개가 같은 기계의 CPU 를 나눠 쓰면서 프레임 타이밍이
 * 흔들리기 때문입니다. 이 검사는 **픽셀까지 같은가**를 묻는 검사라
 * 그 흔들림에 가장 예민합니다.
 *
 * 실측(한 세션에서 **두 번** 당했습니다):
 *   · 폴링 셸 8개를 띄워 둔 채       → 5개 중 1개 실패
 *   · guard·repro·map 을 한 줄로 연달아 → 5개 중 1개 실패
 *   · 조용한 기계에서 단독으로        → **5개 전부 통과**
 *
 * 첫 번째 때는 하마터면 게임 코드를 뜯을 뻔했습니다. 빨간불을 보면
 * **먼저 기계가 조용한지** 확인하십시오:
 *
 *     pgrep -x sleep ; pgrep -f 'node tools'
 *
 * ── 왜 이 검사가 생겼는가 ──────────────────────────────────────────
