  ...
  try {
     ...
     */
    {
      ...
      const popped = await page.evaluate(async () => {
         ...
         */
        /**
         * ── ⏱ **횟수가 아니라 시뮬레이션 시간으로 기다립니다** ────────────
         *
         * 여기 원래 `topUp++ < 80` 이었습니다. 그런데 이 컨테이너는 한
         * 프레임이 0.05 시뮬초라, **80회는 시뮬레이션 시간으로 0.3초쯤**
         * 입니다. 출혈이 터진 직후 다시 0 이 아닌 값까지 쌓기에는 턱없이
         * 모자랐고, 그래서 이 검사 셋이 **간헐적으로** 빨개졌습니다:
         *
         *     때리지 않으면 식는다 — 0 → 0 (앞 타격이 터뜨려서 81대 더 쳐 세움)
         *     쌓이면 게이지가 화면에 뜬다 — 0→안 뜸
         *     차 있는 만큼만 길이가 맞는다 — **표본 없음**
         *
         * `81대` 는 *"81번 때렸다"* 가 아니라 **"80번 돌고 포기했다"** 였습니다.
         * 실제로 이번 회차에 그 빨강을 보고 **제 변경이 깼다고 결론 낼
         * 뻔했습니다**(stash 기준선도 한 번씩만 돌린 것이라 흔들림을 못 봤습니다).
         *
         * 이 저장소가 이미 두 번 적어 둔 규칙 그대로 고칩니다 —
         * **횟수를 세지 말고 찰 때까지**, 그리고 **벽시계가 아니라
         * 시뮬레이션 시계**로.
         */
        let topUp = 0
        const topUpUntil = G.state().elapsed + 4
        while ((G.enemyInfo(e)?.bleed ?? 0) <= 0 && G.state().elapsed < topUpUntil) {
          topUp++
          const i = G.enemyInfo(e)
          if (i) {
            G.setHp(e, 9999)
