...
try {
  ...
  /**
   * ── 🥋 **모아 쓰는 것이 정답이 되면 안 됩니다** ─────────────────────
   *
   * balance.ts 가 이 수치를 선형으로 둔 이유를 이렇게 적어 뒀습니다:
   * *"제곱으로 두면 **기다리기가 정답**이 되어 매 순간의 판단이 사라집니다."*
   *
   * 그 약속이 실제로 지켜지는지는 **부등식 하나**로 답할 수 있습니다.
   * 강타 피해는 `마무리 × (1 + 점수 × damagePerPoint)` 이므로:
   *
   *     1점씩 max 번  =  max × (1 + d)
   *     max점 한 번   =  1   × (1 + max × d)
   *
   * 앞이 크면 *"자주 쓰는 것이 이득"* 입니다. 여기 숫자를 베껴 적지 않고
   * **게임이 내보낸 설정값**으로 계산합니다 — 베껴 적는 순간 이 검사가
   * 또 하나의 진실이 되고, 값을 고치는 날 조용히 옛말이 됩니다.
   *
   * ⚠️ 이 검사가 없어서 실제로 값을 치렀습니다. 같은 자리의 **주석이
   *    정반대로**(*"모아 쓰는 것이 살짝 이득"*) 적혀 있었고, 자동 플레이
   *    봇이 그 문장을 그대로 따라 *"가득 찰 때만 태우는"* 규칙을 썼습니다.
   *    그래서 집중의 절반 넘게가 넘쳐 버려졌고, 그 넘침이 **게임의 결함**
   *    으로 읽힐 뻔했습니다. 계측기가 주석을 베껴 쓰면 주석의 오류가
   *    측정값이 됩니다.
   */
  {
    const split = cfg.max * (1 + cfg.damagePerPoint)
    const hoard = 1 + cfg.max * cfg.damagePerPoint
    console.log(
      `\n  🥋 같은 ${cfg.max}점을 쓰는 두 방법 — 1점씩 ${cfg.max}번 **${split.toFixed(2)}배** · ` +
        `${cfg.max}점 한 번 **${hoard.toFixed(2)}배** (마무리 대비)`,
    )
    check(
      split > hoard,
      '🥋 **모아 쓰는 것이 정답이 아니다** (balance.ts: "기다리기가 정답이 되면 매 순간의 판단이 사라집니다")',
      `나눠 쓰기 ${split.toFixed(2)}배 vs 모아 쓰기 ${hoard.toFixed(2)}배 — ${((split / hoard - 1) * 100).toFixed(0)}% 차이`,
    )
  }
