...
try {
  ...
  /**
   * ── 🏹 **높은 곳에서 아래를 위협하는 적** — 검사를 **못 세웠습니다** ────
   *
   * 이 지도는 높이가 0~8층이고 절벽이 40%인데, 재 보니 수직을 전투에 쓰는
   * 자리가 없었습니다 — 유일한 원거리 적인 궁수(사거리 12m)가 오히려 비탈
   * **아래**(높이 3, 주변 4·6·7)에 서 있습니다. 그러면 궁수는 체력 34짜리
   * 약한 잡몹이고, 층을 여덟 개 만든 뜻이 없습니다. 다크 소울의 저격수,
   * NRFTW 의 위층 전투, 로스트아크의 높이 차가 전부 **높이로 선택을 만드는**
   * 장치인데 여기엔 그 자리가 하나도 없습니다.
   *
   * ── 왜 이번에 못 고쳤는가 (계측기를 또 잘못 골랐습니다) ────────────
   * 높은 자리로 옮기려고 `walkTest` 로 "닿는가"를 훑었는데, 그 함수는
   * **두 점 사이 통행 가능 여부**이지 전 지도 길찾기가 아닙니다. 그래서
   * *"지도의 높은 곳이 전부 못 닿는다"* 는 그럴듯한 결론이 나왔고, 그걸
   * 믿고 지형을 세 번 깎았습니다. 전부 되돌렸습니다.
   *
   * 믿을 수 있는 것은 **이 프로브 자신의 도달성 검사**가 낸 한 줄뿐입니다 —
   * 궁수를 (49,25) 로 옮기자 *"시작 지점에서 모든 배치물에 사다리 없이도
   * 닿는다 — archer(68,48)"* 가 빨개졌습니다. 그 자리는 진짜로 못 갑니다.
   *
   * ── 다음 라운드 ─────────────────────────────────────────────────
   * 검사를 세우기 전에 **자리부터** 있어야 합니다. 이 프로브가 이미 갖고
   * 있는 도달성(flood fill)으로 *"닿으면서 주 동선보다 2층 이상 높은 칸"*
   * 을 뽑고, 하나도 없으면 그때 지형에 계단을 만듭니다. 계단은 한 칸씩
   * 올라가야 하고(`MAX_CLIMB` 1), 폭도 몸 반지름보다 넓어야 합니다.
   *
   * ⚠️ 검사를 하나 **썼다가 지웠습니다**: *"자기 사거리 안의 다른 적보다
   *    2층 이상 높은 적이 있다"*. 그러면 **사거리 4.2m 짜리 잡몹**도 저격수로
   *    세어져 초록이 됐습니다. 원거리는 사거리가 아니라 게임의 정의로 —
   *    `keepDistance > 0`(거리를 유지하려는 적) — 가려야 합니다.
   *    지금 그 정의로 세면 **0개**라 빨간 검사가 되므로, 자리를 만든 뒤에
   *    같이 넣습니다.
   */
