...
try {
   ...
   * ── 📣 **안내가 규칙과 같은 말을 하는가** ──────────────────────────
   *
   * 이 프로브는 규칙을 아홉 가지로 잘 검사하고 있었습니다. 그런데 **화면에
   * 뜨는 문구**는 아무도 안 봤고, 거기가 정확히 썩은 자리였습니다:
   *
   *     실제 규칙 — 예고 중 + **스킬** + 정면
   *     화면 문구 — "정면에서 때려라"          ← 스킬을 안 말함
   *
   * 평타를 정면에 아무리 꽂아도 아무 일도 안 일어납니다. 스킬로 제한한
   * 것은 의도된 설계인데(반격이 사고가 아니라 결단이 되게), **그 설계를
   * 바꾸면서 문구를 안 고쳤습니다.** 이 게임을 만든 사람이 직접
   * *"반격이 잘 모르겠다"* 고 물어서 드러났습니다.
   *
   * ⚠️ 이 저장소는 보통 **문구를 비교하지 않습니다**(문구를 다듬는 날
   *    검사가 조용히 거짓이 되니까). 여기는 예외입니다 — 이 문구의 **일**
   *    자체가 *"스킬이 필요하다"* 를 전하는 것이라, 그 말이 빠지면 문구가
   *    제 일을 안 하는 것입니다. 뜻이 아니라 **의무**를 검사합니다.
   */
  {
    const label = await page.evaluate(() => {
      const c = window.__game.reactionBudget().colors.find((v) => v.label.includes('때려'))
      return c?.label ?? ''
    })
    check(
      label.includes('스킬'),
      '📣 🟢 안내가 **스킬이 필요하다는 것을 말한다** (규칙만 고치고 문구를 안 고치는 일이 실제로 있었습니다)',
      `"${label}"`,
    )
  }
