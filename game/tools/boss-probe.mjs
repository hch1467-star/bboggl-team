...
try {
  ...
  for (let run = 0; run < RUNS; run++) {
    ...
    const r = await page.evaluate(async (fullPressure) => {
       ...
       * `SWING_DUTY` — 벤치가 실제 판에서 잰 「때린 비율」 51% (45~55).
       * 주기는 없습니다 — 짠 주기는 게이트에 걸렸습니다(때리는 자리 주석).
       */
      ...
      while (!done && Date.now() < deadline) {
         ...
         *
         * 주기는 **안 짭니다.** 짜 봤다가 게이트에 걸렸습니다 — 아래
         * 「비율을 보고 누릅니다」 주석에 그 기록이 있습니다.
         *
         ...
         */
        /**
         * ── ⚔️ **창을 짜지 않고 «지금까지의 비율»을 보고 누릅니다** ────
         *
         * 처음엔 주기를 짜서 «51% 동안 누르기»로 했습니다. 게이트가
         * 잡았습니다 — **실제로 휘두른 비율이 67%** 였습니다(68·67·66).
         * 누름과 휘두름은 다릅니다: 한 번 누르면 공격 모션이 창 밖까지
         * 이어집니다. 누르는 비율을 아무리 잘 짜도 휘두르는 비율은
         * 모션 길이에 달려 있고, 그건 무기·콤보 단계마다 다릅니다.
         *
         * 그래서 **비율을 짜지 않고 재서 맞춥니다.** 지금까지 휘두른
         * 비율이 목표보다 낮을 때만 누릅니다. 모션이 길든 짧든, 무기를
         * 바꾸든, 저절로 목표에 붙습니다 — 제가 다시 숫자를 고를 자리가
         * 없어집니다.
         *
         * ⚠️ 한 번 시작한 공격은 안 끊깁니다(누르기를 멈출 뿐). 그래서
         *    쉬는 구간이 저절로 «모션 하나» 단위로 뭉치고, 사람이
  ...
  {
    /**
     * ── ⚔️ **압력을 맞췄다고 «주장»하지 말고 잽니다** ─────────────────
     *
     * 앞 회차에서 저는 *"침대가 자기가 몇 %를 휘둘렀는지 세서 같이
     * 냅니다"* 라고 적어 놓고 **값을 만들기만 하고 출력을 안 했습니다.**
     * 아무도 못 보는 값은 없는 값입니다.
     *
     * 목표는 벤치 실측 **51% (45~55)** 입니다. 게이트로 두는 이유:
     * 못 맞췄으면 아래 구간 «길이»들은 여전히 옮겨지지 않는 숫자이고,
     * 그걸 모르는 채로 읽으면 이번 회차에 네 번 한 실수를 다섯 번째로
     * 하게 됩니다.
     *
     * ⚠️ 누른 비율이 아니라 **실제로 휘두른 비율**입니다(공격 모션이
     *    누름보다 깁니다). 게임의 `player.swinging` 을 씁니다.
     */
    const got = ok.map((s) => s.swingPct ?? 0)
    const m = got.length ? [...got].sort((a2, b2) => a2 - b2)[got.length >> 1] : 0
    if (PRESSURE_FULL) {
      console.log(`     ⚔️ 압력  **옛 압력(쉬지 않고 때리기)** — 실제로 휘두른 비율 ${m}%`)
    } else {
      check(
        m >= 45 && m <= 55,
        '⚔️ **침대가 실제로 사람만큼 때렸다** (벤치 실측 51% · 45~55% · 아래 «길이»들의 게이트)',
        `실제로 휘두른 비율 ${m}% (판별 ${got.join(' · ')})`,
      )
    }
  }
  ...
  if (ok.length) {
    ...
    {
      /**
       * ── 🎓 **1단계는 «체력»이 아니라 «가르치기»가 길이를 정합니다** ──
       *
       * 「마지막 구간이 가장 길다」가 3.5배로 빨개졌는데, 검사가 스스로
       * *"분모가 눌린 값"* 이라고 경고했습니다. 그 분모를 갈라 봅니다.
       *
       *     1단계에서 체력을 실제로 깎는 데    1.3초
       *     1단계에서 색 3가지를 다 보는 데    7.2초   ← 5.5배
       *
       * 두 시계가 5배 넘게 어긋나 있고, 그 틈을 **붙듦**(색을 다 볼
       * 때까지 체력을 경계 아래로 안 내려 보내기)이 메웁니다. 결과적으로
       * **1단계의 81%가 붙듦**입니다 — 플레이어가 때리는데 체력이 안
       * 줄어드는 시간이요.
       *
       * ⚠️ **여기서 판정은 안 답니다.** 「몇 %면 너무 많은가」는 설계
       *    결정이지 계기가 정할 값이 아니고, 문턱을 지어내는 것이 이번
       *    회차에 네 번 데인 그 모양입니다. 대신 **숫자를 눈에 띄게**
       *    둡니다 — 다음에 이 배수를 볼 사람이 분자(3단계)부터 만지지
       *    않도록.
       *
       * ⚠️ 이 침대는 **쉬지 않고 때립니다.** 사람은 더 느리므로 붙듦이
       *    이만큼 안 걸릴 수 있습니다. 즉 이 81%는 **상한**이지 사람이
       *    겪는 값이 아닙니다 — 「봇이 잘 쓰는 것과 사람이 쓸 수 있는
       *    것은 다르다」.
       */
      const h = mid(ok.map((s) => s.heldT))
      const r1 = mid(ok.map((s) => s.raceT))
      const tot = h + r1
      console.log(
        `     🎓 1단계의 길이를 정하는 것 — 체력 깎기 ${r1.toFixed(1)}초 vs 가르치기 ${tot.toFixed(1)}초` +
          ` (${(tot / Math.max(0.01, r1)).toFixed(1)}배) → **붙듦이 1단계의 ${Math.round((h / Math.max(0.01, tot)) * 100)}%**`,
      )
      /**
       * ── ⚠️ **이 숫자는 이 침대 밖으로 안 나갑니다 (재고 확인했습니다)** ──
       *
       * 위 81% 를 보고 저는 *"보스가 자기를 소개할 시간이 없다"* 는
       * 결론을 커밋했습니다. 그 뒤 **실제 판에서 같은 것을 재** 봤습니다
       * (벤치 🎓 학습 잠금 · 게임이 직접 세는 `phase1ClampT`):
       *
       *     침대     붙듦이 1단계의 **81%**
       *     실제 판  잠금이 문 시간 **0.0초 (0~0.2)** — 보스전의 **0%**
       *
       * **실제로는 한 번도 안 뭅니다.** 보스는 체력이 경계에 닿기 전에
       * 색 셋을 자연스럽게 다 보여줍니다.
       *
       * 이유는 압력입니다. 이 침대는 기력 무한으로 쉬지 않고 때려
       * **화력 118/초**를 냅니다. 실제 판은 보스 620을 약 26초에 잡으니
       * **약 24/초** — 거의 5배입니다. 잠금은 빨리 깎을수록 더 뭅니다.
       *
       * ⭐ **「일정한 압력」은 판끼리 견주라고 만든 것이지, 그 압력이
       *    실제와 같다는 뜻이 아닙니다.** 이 침대의 구간 «길이»는
       *    그래서 실제 판으로 안 옮겨집니다 — 견주는 데만 씁니다.
       */
      console.log(
        `        ⚠️ 실제 판의 같은 잠금은 **0~1%** 입니다(벤치 🎓 학습 잠금).` +
          ` 여기가 높은 것은 침대가 **순간이동으로 붙어 안 빗나가서** 때리는 시간당 효율이 높기 때문입니다` +
          ` — 때리는 «비율»은 맞췄지만 «효율»은 아직 아닙니다.` +
          ` **이 %를 설계 문제로 읽지 마십시오** (한 번 그렇게 읽고 되돌렸습니다).`,
      )
    }
