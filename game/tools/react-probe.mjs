      ...
      if (!spawnFailed && rows.length > 0) {
        ...
        check(
          ...
          `${fastest.from} ${fastest.id} · 예고 ${fastest.windup.toFixed(2)}초` +
            ...
            /**
             * ── ⏱ **「읽자마자 구르면 안 된다」 — 이건 고장이 아닙니다** ──
             *
             * 이 꼬리표가 여러 회차 동안 떠 있었고, **아무도 결정을 안
             * 했습니다.** 지금 결정하고 근거를 남깁니다.
             *
             * 실측(수문장 🔵 `boss_bind`):
             *     무적 창이 열리는 시각 = 예고 0.83 − iFrameEnd = **0.53초**
             *     색을 알아보는 반응 예산            = **0.482초**
             *     → 창이 **0.05초 늦게** 열립니다
             *
             * 소울류에서 *"읽자마자 구르면 안 되는 것"* 은 고장이 아니라
             * **회피의 본질**입니다. 반응이 아니라 **타이밍**을 요구하는
             * 것이 이 장르가 파는 것이고, 이 게임의 다른 색도 같습니다.
             *
             * ── 진짜 문제는 다른 데 있었습니다 ────────────────────────
             * 화면이 **「언제」를 안 알려 줬습니다.** 예전 예고는 투명도만
             * 차올라서 *"점점 위험해진다"* 는 말했지만 *"지금 0.2초
             * 남았다"* 는 말하지 않았습니다. 타이밍을 요구하면서 타이밍을
             * 볼 방법을 안 주면, 플레이어는 *"내가 못 피했네"* 가 아니라
             * **"언제 눌러야 하는지 몰랐네"** 라고 말하게 됩니다 —
             * 기둥 2가 금지한 쪽입니다.
             *
             * ── 그 구멍은 메워졌습니다 ────────────────────────────────
             * **차오르는 부채꼴**(visuals.ts `telegraphFill`)이 가운데에서
             * 자라 바깥 선에 닿는 순간이 판정입니다. `npm run parry` 가
             * 그 말이 사실인지 재고 있습니다(창 안에서 0.99까지 참).
             * 길이는 진하기와 달리 **눈이 절대값으로 읽습니다** — 비교
             * 대상(바깥 선)이 늘 같은 화면에 있으니까요.
             *
             * 나눔이 이렇게 정리됩니다:
             *   · **기계** — 몇 초 남았는지 재서 **보여 줍니다**
             *   · **사람** — 그걸 보고 **언제 누를지 정합니다**
             *
             * 차오름이 답을 대신 눌러 주지는 **않습니다.** 구르기는
             * `iFrameStart` 만큼 **앞서** 눌러야 하므로 여전히 배워야
             * 합니다. 다만 이제 **볼 수 있는 시계**를 보고 배웁니다.
             *
             * ⚠️ 그래서 이 꼬리표는 **경고가 아니라 설명**입니다. 이걸
             *    없애려고 무적 값이나 반응 예산을 만지지 마십시오 —
             *    타이밍을 없애면 이 색의 답이 「보이면 누른다」가 되고,
             *    그러면 🔵 는 색일 이유가 없어집니다.
             */
            (openAt > budget.choice
              ? ` (반응 예산보다 ${(openAt - budget.choice).toFixed(2)}초 늦게 열림 —` +
                ` **읽자마자 구르면 안 됩니다.** 의도된 것입니다 — 위 ⏱ 주석 참고)`
              : '') +
            `\n                 ${rows.join(' · ')}` +
            note,
        )
