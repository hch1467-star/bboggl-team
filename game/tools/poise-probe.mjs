...
try {
   ...
   *
   * ── 일곱 판의 실제 분포 ─────────────────────────────────────────
   *     가만히       6·6·5·4·5·6·5  → 가운데값 5
   *     계속 때릴 때 1·2·2·2·2·3·2  → 가운데값 **2**
   *
   * 가운데값이 문턱(2)과 **같습니다.** 동점은 이 회차에 여러 번 물린
   * 자리라, **문턱을 넘은 판 수**를 로그에 같이 찍습니다 — 여기서는
   * **6/7** 이라 동전 던지기가 아닙니다. 가운데값만 보면 «아슬아슬»
   * 로 읽히고, 판 수를 보면 «넉넉»으로 읽힙니다. 두 수가 다른 이야기를
   * 하므로 **둘 다** 보여 줍니다.
   *
   * ⚠️ 이 수가 4/7 아래로 내려가면 가운데값이 1이 되어 빨개집니다.
   *    그때는 자리가 아니라 **강인도 값**을 보십시오.
   */
  const MIN_SWINGS = 2
  check(
    grunt.idle.swings > 0,
  ...
  check(
    ...
    `${grunt.idle.swings}회 → ${grunt.spam.swings}회 (${(
      ...
      100
    ).toFixed(0)}% 남음 · ${REPEATS}판 가운데값) · **문턱을 넘은 판 ${
      trials.filter((t) => t.spam.swings >= MIN_SWINGS).length
    }/${REPEATS}** · 비율은 기록이고 판정은 절대 횟수로 합니다`,
  )
  ...
  if (gpRows.length === 3 && gpRows.every((g) => g.dmg > 0 && g.poiseMax > 0)) {
     ...
     */
    /**
     * ⚠️ **양쪽을 다 봅니다 — 한쪽만 보면 «고치는 법»이 하나 더 생깁니다.**
     *
     * 처음엔 위쪽(«한 방에 안 무너진다»)만 검사했습니다. 그런데 그러면
     * `poise: 0.01` 로 놔도 초록입니다 — **막을 이유가 없는 게임**이 되는
     * 쪽으로도 통과합니다. 이 저장소가 이미 적어 둔 것: *"아무도 못 넘는
     * 문턱은 눈금이 아니라 벽이다."* 그 반대편도 똑같습니다.
     *
     * 띠를 **2~4번**으로 둡니다. 두 번 미만이면 한 번의 요행이 구간을
     * 끝내고, 네 번을 넘으면 «가장 어려운 입력»의 보상이 평타만도 못합니다.
     */
    /**
     * ⚠️ **처음엔 «배»로 쟀고, 그래서 빨강이 났습니다 — 검사가 틀렸습니다.**
     *
     *     가드 60 vs 보스 105  →  1.75  →  `>= 2` 를 못 넘음 ❌
     *
     * 그런데 **1.75번 막는 사람은 없습니다.** 사람이 겪는 수는 «두 번»이고,
     * 실수(實數) 비는 그 사이 어디쯤인지를 말할 뿐입니다. 재는 단위가
     * 사람이 낼 수 없는 단위면, 그 눈금은 게임을 재는 게 아닙니다.
     *
     * ⚠️ **값을 초록으로 만들려고 문턱을 내린 것이 아닙니다.** 문턱을 그대로
     *    두고 **단위를 바꿨습니다**. 두 수를 나란히 찍어 두는 이유가
     *    그것입니다 — 1.75 를 숨기면 이 판단을 나중에 되짚을 수가 없습니다.
     *
     * 띠를 **2~4번**으로 둔 근거는 견줄 것이 있어서입니다:
     *
     *     강타 54.6 → 보스에 **2번**   (집중을 태워서 내는 한 방)
     *     가드 60   → 보스에 **2번**   ← 같은 급, 조금 더 셈
     *
     * 위로 넘으면 «한 번의 요행이 구간을 끝내는» 게임이 되고, 아래로
     * 넘으면 «가장 어려운 입력의 보상이 평타만도 못한» 게임이 됩니다.
     */
    const ratio = gp.boss.dmg > 0 ? gp.boss.poiseMax / gp.boss.dmg : Infinity
    const need = Number.isFinite(ratio) ? Math.ceil(ratio) : Infinity
    const detail = `가드 ${gp.boss.dmg} vs 보스 강인도 ${gp.boss.poiseMax} — **${need}번** 막아야 무너집니다 (실수 비 ${ratio.toFixed(2)})`
    check(
      need >= 2,
      '🛡 **저스트 가드 한 번으로 보스가 즉시 무너지지는 않는다** (막기는 «수단»이지 «정답»이 아닙니다)',
      need < 2
        ? `${detail} — 단위가 뒤바뀐 값일 수 있습니다(trauma 자리에 강인도 값)`
        : detail,
    )
    check(
      need <= 4,
      '🛡 **저스트 가드가 «막을 이유»는 되어야 한다** (0.18초 창을 읽고도 안 무너지면 그 입력은 없는 것)',
      need === Infinity ? '값이 0입니다' : `${detail} (허용 2~4번)`,
    )
  }
   ...
   * ── 🎯 **간파할 «창이 존재하는» 공격인가** ─────────────────────────
   *
   * 이 검사가 없어서 **아무도 못 넘는 문턱**을 직접 만들었습니다. 간파는
   * «예고의 마지막 구간에 강타를 넣는 것»인데, 강타는 눌러서 닿기까지
   * **0.42초**가 걸립니다. 그러니 예고가 그보다 짧으면 **어떤 손으로도
   * 못 냅니다.** 세 번을 재고 커밋까지 한 뒤에야 알았습니다 — 산수 한
   * 줄이면 잡혔을 일입니다.
   *
   * ⚠️ **산수를 검사가 다시 하지 않습니다.** 게임이 냅니다(`punishable`).
   *    이 회차의 교훈이 정확히 «곱셈을 두 곳에 두면 갈라진다» 였습니다.
   *
   * 간파 불가인 공격이 있는 것 자체는 설계 결정일 수 있습니다(빠른 잽은
   * 못 읽는 게 맞습니다). 다만 **모르고 있으면 안 됩니다** — 그래서
   * 빨강이 아니라 **이름을 대어** 냅니다.
   */
  const pun = await page.evaluate(() => window.__game.punishable())
  check(pun.length > 0, '🎯 공격 목록을 실제로 읽었다 (아래 판정의 게이트)', `${pun.length}개`)
  if (pun.length > 0) {
    const dead = pun.filter((a) => !a.ok)
    console.log(
      `\n  🎯 **간파할 창이 있는 공격** — ${pun.length - dead.length}/${pun.length}개` +
        ` (강타가 닿기까지 ${pun[0].need}초)` +
        (dead.length > 0
          ? `\n     ⚠️ 창이 없는 공격: ${dead.map((a) => `${a.id}(예고 ${a.windup}초)`).join(' · ')}`
          : ''),
    )
    /**
     * 문턱을 «하나라도 있으면»이 아니라 **과반**으로 둔 이유: 하나만
     * 간파되는 게임은 그 규칙을 배울 기회가 없습니다. 배움에는 반복이
     * 듭니다(이 저장소의 4색 예고 설계 노트와 같은 근거).
     */
    check(
      pun.length - dead.length > pun.length / 2,
      '🎯 **적 공격의 과반이 간파할 수 있다** (하나만 되면 배울 기회가 없습니다)',
      `${pun.length - dead.length}/${pun.length}개 · 강타가 닿기까지 ${pun[0].need}초`,
    )
  }
