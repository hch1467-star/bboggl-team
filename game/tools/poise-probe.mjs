...
try {
  ...
  if (Object.values(gp).every((g) => g.dmg > 0 && g.poiseMax > 0)) {
     ...
     */
    const need = gp.boss.dmg > 0 ? gp.boss.poiseMax / gp.boss.dmg : Infinity
    check(
      need >= 2,
      '🛡 **저스트 가드 한 번으로 보스가 즉시 무너지지는 않는다** (막기는 «수단»이지 «정답»이 아닙니다)',
      `가드 ${gp.boss.dmg} vs 보스 강인도 ${gp.boss.poiseMax}` +
        (need < 2
          ? ` — **${(1 / need).toFixed(1)}배**. 단위가 뒤바뀐 값일 수 있습니다(trauma 자리에 강인도 값)`
          : ` — ${need.toFixed(1)}번 막아야 무너집니다`),
    )
    check(
      need <= 4,
      '🛡 **저스트 가드가 «막을 이유»는 되어야 한다** (0.18초 창을 읽고도 안 무너지면 그 입력은 없는 것)',
      `${need === Infinity ? '값이 0입니다' : `${need.toFixed(1)}번 막아야 무너집니다 (허용 2~4번)`}`,
    )
  }
