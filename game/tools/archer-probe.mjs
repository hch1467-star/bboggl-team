...
try {
   ...
   * ── 🎟 **토큰이 진짜 병목인가** ───────────────────────────────────
   *
   * 이 자리에 원래 「토큰 탓 57%」라는 숫자가 있었고, 그대로 읽으면
   * 처방은 하나뿐이었습니다 — **토큰을 늘려라.** 그 변경은 공짜가
   * 아닙니다. 소스에 근거가 적혀 있습니다: *"쿼터뷰에서 적이 겹치면 색
   * 예고가 서로를 가립니다. 공격 토큰으로 동시 예고를 2개로 줄였지만,
   * 2개도 겹치면 하나는 안 보입니다."* 즉 기둥 2(예고를 읽어 대응한다)를
   * 직접 흔듭니다.
   *
   * 그래서 「몇 %」가 아니라 **「한 발을 잃었는가」**를 묻습니다. 준비된
   * 채 토큰을 기다린 시간이 **한 바퀴보다 길면** 그 사이 한 발이 통째로
   * 사라진 것입니다. 그보다 짧으면 기다리긴 했어도 발수는 그대로입니다 —
   * 「아무도 못 넘는 문턱은 눈금이지 벽이 아니다」의 짝으로,
   * **고쳐도 안 바뀌는 몫을 병목이라 부르지 않습니다.**
   */
  const tokenOnly = (crowd.why ?? {})['토큰만'] ?? 0
  check(
    tokenOnly < A.attackCycle,
    '🎟 **토큰을 기다리느라 한 발을 통째로 잃지는 않았다** (잃었다면 고칠 곳은 토큰 회전입니다)',
    `준비된 채 기다린 ${tokenOnly.toFixed(1)}초 vs 한 바퀴 ${A.attackCycle.toFixed(2)}초` +
      ` · 실제 ${crowd.tele}발 / 이 시간의 천장 ${(crowd.seconds / A.attackCycle).toFixed(1)}발`,
  )
  check(
    crowd.tele >= 1,
