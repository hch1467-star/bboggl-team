...
try {
    ...
    while (now() - t0 < LIMIT) {
      ...
      if (near && reachable && near.dist < engageRange) {
        ...
        if (near.dist > 2.2) {
           ...
           * 🥋 **한 점이라도 있으면 태웁니다.**
           *
           * ❌ 원래는 *"가득 찼을 때만"* 이었습니다. 이유로 적힌 것은
           *    *"봇은 지금 태울까 더 모을까를 판단하지 못하니 가장 단순한
           *    규칙을 쓴다"* 였는데, **그 단순한 규칙이 하필 가장 나쁜
           *    규칙**이었습니다. 강타 피해는 `마무리 × (1 + 점수 × 0.85)` 라
           *    기반값 1이 매 발동마다 붙습니다:
           *
           *        1점씩 세 번 = 5.55배   ·   3점 한 번 = 3.55배
           *
           *    모아 쓰면 **56% 손해**입니다. 그런데 balance.ts 주석에는
           *    *"모아 쓰는 것이 살짝 이득"* 이라고 적혀 있었고, 이 봇이 그
           *    문장을 그대로 따랐습니다. 그 결과 집중의 절반 넘게가 넘쳐
           *    버려졌고, 저는 그 넘침을 **게임의 결함**으로 읽을 뻔했습니다.
           *
           ...
           */
          if (G.focusInfo().focus >= 1) tap('Mouse2')
  ...
  /**
   * ── ☠️ **죽음을 전제로 만든 장치들이 값을 냈는가** ──────────────────
   *
   * ── 왜 한 줄로 묶는가 ────────────────────────────────────────────
   * 이 게임에는 *"죽고 다시 온다"* 를 전제로 만든 장치가 여럿입니다 —
   * 화톳불(체크포인트) · 지름길 사다리 · 죽으면 잃는 불티 · 성수병.
   * 그런데 그 값들이 **보고서 여기저기에 흩어져** 있어서, 넷을 나란히
   * 놓기 전에는 *"이 장치들이 이번 판에 값을 냈나"* 를 물을 수가
   * 없었습니다.
   *
   * 실제로 네 판을 손으로 뒤져서야 알았습니다:
   *
   *     네 판 연속 **사망 0회** · 70m 짜리 지름길 사용 **0회**
   *     보스전 **받은 피해 0** · 기력이 모자랐던 시간 **0%**
   *
   * 「사다리를 안 쓴다」는 배치나 안내의 문제가 아니었습니다. **죽지
   * 않아서**입니다 — 다크 소울에서 지름길이 값진 이유는 죽고 돌아오기
   * 때문이고, 한 번에 끝까지 가는 판에서는 아낄 기회가 아예 안 옵니다.
   * 사다리를 옮기거나 안내를 키워도 **아무것도 안 바뀝니다.**
   *
   * ⚠️ **봇의 0사망을 난이도 근거로 쓰지 마십시오.** 봇은 숫자를 읽고
   *    완벽한 타이밍에 구릅니다 — 사람이 안 죽는다는 뜻이 아닙니다.
   *    이 저장소가 이미 적어 둔 경계입니다(`ab.mjs`: *"봇의 정책이
   *    사람과 닮지 않은 기능은 여기서 답이 안 나옵니다"*). 이 줄은
   *    **판정이 아니라 장부**이고, 말하는 것은 하나뿐입니다 —
   *    *"이 판에서 그 장치들에 기회가 있었나."*
   */
  console.log(`  사망       ${log.deaths}회`)
  console.log(
    `  ☠️ 되돌아옴  사망 ${log.deaths}회 · 지름길 ${log.ladderOpen}/${log.ladderTotal} · ` +
      `성수병 ${log.vialsUsed}개 · 휴식 ${log.restCount}회` +
      (log.deaths === 0
        ? `\n              ⚠️ **한 번도 안 죽었습니다** — 지름길·화톳불·불티 손실은 이 판에서 ` +
          `**값을 낼 기회가 없었습니다.** 안 쓰인 것이 아니라 **쓸 일이 없었던 것**입니다`
        : ''),
  )
  ...
  console.log(
    ...
    (() => {
      ...
      const lines = [...by.entries()]
        ...
        .map(
          ([id, e]) =>
            `               ${id.padEnd(13)}${String(e.n).padStart(3)}회 · 잡힘 ${String(e.hit).padStart(3)}(${Math.round(
              (e.hit / e.n) * 100,
      ...
      /**
       * ── ⚠️ **이 칸은 「맞았다」가 아니라 「잡힐 수 있었다」입니다** ──
       *
       * 오늘 실험대에서 말이 안 되는 줄을 봤습니다 —
       * `받은 피해 0 · 판정 적중`. 두 값이 **다른 것을 세고** 있었습니다:
       *
       *   · 장부의 `hit` — 판정이 살아 있는 **매 프레임**을 보고, 한
       *     프레임이라도 *"도형 안 + 무적 아님"* 이면 참
       *   · 실제 피해   — 그 휘두름의 **피해 프레임 한 번**에만 들어감
       *
       * 구르기 무적이 **피해 프레임만 덮고 나머지 판정 프레임은 못 덮으면**
       * 장부는 적중이라 하고 체력은 안 깎입니다. 둘 다 맞는 말이고 묻는
       * 것이 다릅니다.
       *
       * 실제로 이 판이 그랬습니다: **적중 76%** 인데 **보스전 받은 피해 0**.
       * 「적중」이라고 적어 두면 읽는 사람은 *"적이 잘 맞히고 있다"* 로
       * 읽고, 그 위에서 밸런스를 판단하게 됩니다.
       *
       * 그래서 이름을 사실대로 바꿉니다. 실험대(`react`)는 오늘 고쳤는데
       * **여기는 안 고쳤습니다** — 같은 값을 두 곳이 다르게 부르고
       * 있었습니다.
       *
       * ⚠️ 피해가 실제로 들어갔는지는 이 장부만으로 못 셉니다(피해 칸이
       *    없습니다). 세려면 `SwingRecord` 에 피해를 실어야 하는데, 그건
       *    판정 자리에서 한 번 더 재는 일이라 **필요해질 때** 하십시오.
       *    지금 고치는 것은 **이름**이고, 이름이 틀린 것이 값이 없는 것보다
       *    나쁩니다.
       */
      ...
      return (
        ...
        `               ⚠️ 「잡힘」은 **맞았다가 아니라 잡힐 수 있었다**입니다(위 주석)\n` +
        `               합계 — 잡힘 ${hit}(${Math.round((hit / tot) * 100)}%) · 사거리 ${far}(${Math.round(
          (far / tot) * 100,
