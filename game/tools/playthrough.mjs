...
try {
  ...
  const log = await page.evaluate(
    ...
    /**
     * 🥋 **강타를 태우는 문턱 — 이 한 곳에만 적습니다.**
     *
     * 쓰는 자리가 둘입니다: 실제로 누르는 자리와, 보고서에 적어 내보내는
     * 자리. 값을 두 곳에 적으면 갈라지고, 갈라진 것을 알려 주는 것은
     * 아무것도 없습니다 — 벤치가 정확히 그렇게 **옛 규칙을 화면에 찍고
     * 있었습니다**(아래 `heavyBurnAt` 주석에 그 사고 기록이 있습니다).
     *
     * 값이 1인 근거: 강타 피해는 `마무리 × (1 + 점수 × 0.85)` 라 기반값 1이
     * **매 발동마다** 붙습니다. 1점씩 세 번 = 5.55배 vs 3점 한 번 = 3.55배 —
     * 모아 쓰면 56% 손해입니다.
     */
     ...
     * 붕괴 횟수가 같아도 무방비 1.9초(`POISE.brokenTimeBoss`)가 차지하는
     * **비중**은 구간 길이에 따라 달라집니다 — 횟수만 세면 안 보입니다.
     *
     * ── ❌ **여기 적었던 예상은 틀렸습니다** (재고 나서 고칩니다) ────
     * 이 자리에 *"10.3초 구간에서 18%, 4.4초 구간에서 **43%**"* 라고
     * 적어 두었습니다. 붕괴 1회 × 1.9초를 구간 길이로 나눈 **산수**였고,
     * 재 본 값이 아니었습니다. 눈금을 만들어 놓고 눈금을 읽기도 전에
     * 답을 적은 셈입니다.
     *
     * 4판 벤치가 실제로 낸 값:
     *
     ...
     *     3단계 6.4초 · 누워 있던 비중 **2%**
     *
     * 예상과 반대로 **뒤로 갈수록 줄어듭니다.** 산수가 빠뜨린 것은
     * **처형**입니다 — 무방비 1.9초를 끝까지 쓰는 게 아니라, 처형이
     * 들어가면 그 자리에서 끝납니다(구간마다 처형이 약 1회씩 있습니다).
     * 즉 「무방비 창이 뒤로 갈수록 커진다」는 갈래는 **탈락**입니다.
     *
     * 이 칸의 값어치가 바로 그것이었습니다 — 제 예상을 지웠습니다.
     *
    ...
    while (now() - t0 < LIMIT) {
...

      {
        ...
        for (const t of G.threats(400)) {
          ...
          /**
           * 🚶 **걸어야 하는 거리는 «게임이 판정에 쓴 그 값»을 받습니다.**
           *
           * `enemyAI` 는 직선이 아니라 걸어야 하는 거리로 깨웁니다(벽 건너
           * 적이 깨어나 영원히 벽을 향해 걷던 사고 때문입니다). 그래서
           * 직선 4.4m 라도 **돌아가야 하면** 안 깰 수 있습니다.
           *
           * ── ⚠️ 이 자리가 **틀린 라벨을 찍고 있었습니다** ────────────────
           *     (61,27) 가장 가까이 17.4m · 깨는거리 안 134프레임 ·
           *     깨어 있던 0프레임 — *"깨는 거리 안에 들어갔는데 안 깼다"*
           *
           * 게임은 멀쩡했습니다. **걸어서 24m** 라 19m 밖이었고, 안 깨는 게
           * 맞습니다. 틀린 것은 위 줄이 **직선(`t.dist`)** 을 깨는 거리와
           * 견준 것입니다. 이 저장소가 직선/경로를 혼동해 낸 네 번째
           * 사고이고, 처방은 앞의 셋과 같습니다 — **프로브가 자기 자로
           * 다시 재지 말고 게임이 쓴 값을 읽는다**(`threats().walk`).
           *
           * 덤으로 `pathStep` 호출이 사라졌습니다. 그건 흐름장을 **다시
           * 세워서** 안내까지 흔드는 무거운 호출이었는데, 이제 매 프레임
           * 한 칸 읽기입니다 — 그래서 「가장 가까이」도 **판 전체의 최소**로
           * 정직해졌습니다(예전엔 직선이 가장 가까웠던 순간의 값이었습니다).
           */
          const walk = t.walk === null || t.walk === undefined ? t.dist : t.walk
          if (walk < rec.nearWalk) rec.nearWalk = walk
          if (walk <= archerWake) rec.inWake++
       ...
       */
      {
        ...
        if (parts && parts.spare.length > 0) {
          ...
          fit: for (const part of parts.spare) {
            /**
             * ── ⚖️ **대가를 요구하는 부품은 안 집습니다** ──────────────
             *
             * 72개 중 **14개**가 교환입니다 — 「관통」은 판정이 원형이
             * 되는 대신 **피해 −25%**, 「확산」은 각도 +72°에 **피해 −30%**.
             *
             * 사람은 그 대가를 **다른 데서 회수할 계획**이 있을 때만
             * 집습니다(적이 줄지어 설 때, 한 방을 몰아칠 때). 봇에게는
             * 그 계획이 없습니다. 눈감고 집으면 **성장할수록 약해지고**,
             * 그러면 잰 값이 게임이 아니라 **봇의 무지**를 재게 됩니다.
             *
             * 실제로 그랬습니다 — 성장을 켜자 받은 피해가 53 → 128 로
             * 늘었습니다. 「봇의 습관을 게임의 성질로 착각하지 않는다」.
             *
             * ⚠️ **이게 «최적 플레이»는 아닙니다.** 순수 이득만 집는 손은
             *    사람보다 단순합니다. 다만 «손해를 눈감고 집는 손»보다는
             *    실제 플레이에 가깝고, 무엇보다 **왜 그렇게 골랐는지
             *    설명할 수 있습니다.**
             */
            if (G.partIsTradeoff?.(part.id)) continue
      ...
      if (near && reachable && near.dist < engageRange) {
         ...
         * ── 🎯 **간파 — 구르기보다 «먼저» 봅니다** ────────────────────
         *
         * ⚠️ 이 가지를 처음엔 **공격 가지 안**(맨 아래)에 뒀습니다. 그리고
         *    벤치가 세 판 연속 **간파 0회**를 냈습니다. 가지도 자원도
         *    멀쩡했는데, 원인은 **순서**였습니다:
         *
         *      창이 열리는 때 = 적이 예고 중인 때
         *      그런데 적이 예고 중이면 봇은 **가드/구르기 가지에서 이미
         *      `continue`** 해 버립니다. 공격 가지까지 내려오질 않습니다.
         *
         *    즉 간파는 **정의상 도달 불가능한 자리**에 있었습니다. 앞선
         *    회차에서 이 규칙이 «닿는 창»이라 물리적으로 불가능했던 것과
         *    **똑같은 실수를 순서로** 한 번 더 한 것입니다.
         *
         * ⭐ **새 가지를 넣을 때는 «그 가지에 닿는 길이 있는가»부터
         *    봐야 합니다.** 조건을 아무리 잘 써도 위에서 `continue` 하면
         *    그 코드는 없는 것과 같고, 벤치는 그것을 «규칙이 안 통한다»로
         *    적습니다.
         *
         * 순서를 이렇게 둡니다 — 🟢 반격 · 🔴 가드는 **색 전용 답**이라
         * 그대로 앞에 두고, 간파는 그 둘이 안 잡은 예고에 답합니다.
         * 구르기는 «답이 없을 때»의 답이므로 맨 뒤가 맞습니다.
         */
    ...
    return {
       ...
       *
       * ⚠️ 벤치가 이 규칙을 **베껴 적고 있었습니다.** 화면에 매 판
       *    *"봇은 «가득 찼을 때만» 태웁니다"* 라고 찍었는데, 봇은 언젠가
       *    `focus >= 1` 로 바뀌어 있었고 벤치의 해설만 옛 규칙에 멈춰
       *    있었습니다. 그 거짓 문장이 근거가 되어, 「강타 1/112」에
       *    *"봇 습관일 수 있으니 결론을 미룬다"* 는 유보를 **두 커밋에
       *    걸쳐** 달았습니다 — 무효인 근거로 미룬 결론이었습니다.
       *
       * 같은 규칙을 두 파일에 적으면 언젠가 한쪽만 고쳐집니다. 그래서
       * **규칙을 가진 쪽이 값을 냅니다.**
       */
      heavyBurnAt: HEAVY_BURN_AT,
      /**
       * 🎯 **간파를 «몇 번 시도했나»** — 게임이 세는 «몇 번 성공했나»
...
` +
  ...
  /**
   * ── 🪂 **밀어서 떨어뜨리기 — 재 놓고 안 읽던 값** ────────────────────
   *
   * `falls: G.fallLog()` 는 오래전부터 판마다 모아 두고 있었는데 **화면에
   * 한 번도 안 찍혔습니다.** 이 저장소가 이번 세션에만 네 번 만난 모양
   * 입니다 — 「안 읽은 값은 없는 값이다」.
   *
   * 이 숫자가 답하는 것: 이 게임에는 **절벽을 무기로 쓰는 문법**이 코드에
   * 이미 있습니다(physics.ts 낙하 → 피해 + `breaksPoise`, 그리고 적에게는
   * 낙하 공격 창을 안 열어 **절벽은 플레이어의 도구**로 남겨 뒀습니다).
   * 세키로의 발차기, 오공의 밀치기, 젤다의 「굴러 떨어지는 바위」가 파는
   * 그 재미입니다. 그런데 **판에서 실제로 일어나는지**는 아무도 안 봤습니다.
   *
   * 0 이면 셋 중 하나입니다 — 넉백이 약하거나 · 동선 옆에 단차가 없거나 ·
   * 봇이 그런 자리에서 안 싸우거나. 갈라 보려면 먼저 **셈**이 필요합니다.
   */
  {
    const f = log.falls
    console.log(
      `             🪂 절벽 — ` +
        (f
          ? `내가 떨어진 것 ${f.player}회 · **적이 떨어진 것 ${f.foe}회**` +
            (f.foe > 0
              ? ` (평균 ${(f.foeSteps / f.foe).toFixed(1)}단 · ${Object.entries(f.byKind)
                  .map(([k, n]) => `${k} ${n}`)
                  .join(' · ')})`
              : ' — **한 번도 안 일어났습니다**')
          : '**장부 없음**'),
        ...
        : a.winding > 0
          ...
          : a.woke
            ...
            : a.inWake > 0
              ? '깨는 거리 안에 들어갔는데 **안 깼다**'
              : /**
                 * ⚠️ **여기서 두 갈래로 나눕니다 — 처방이 다릅니다.**
                 *   · 직선으로도 멀었다   → **배치가 동선에서 멀다**(지도)
                 *   · 직선은 닿았는데 걸어서 멀었다
                 *                       → **지형이 돌아간다**(그 사이에 벽·낭떠러지)
                 * 앞은 궁수를 옮기는 이야기이고, 뒤는 옮겨도 안 바뀝니다.
                 * 한 칸에 담아 뒀더니 실제로 거꾸로 읽었습니다 — 걸어서
                 * 24m 인 것을 「17.4m 인데 안 깬다」로 읽고 게임을 의심했습니다.
                 */
                Number.isFinite(a.near) && a.near <= (r.wake ?? 0)
                ? '직선으로는 닿았지만 **걸어서는 깨는 거리 밖**이었다 (지형이 돌아갑니다)'
                : '**깨는 거리 안에 든 적이 없다**'
      console.log(
