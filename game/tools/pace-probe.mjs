...
try {
  ...
  if (runs.length < 2) {
     ...
     *     → **이 손으로는 boss_hook 이 어느 단계에도 안 나옵니다(0회).**
     *
     * ⚠️ **「실전에 없다」로 읽으면 안 됩니다 — 제가 한 번 그렇게 적었고
     *    틀렸습니다.** 자동 플레이 9판을 세어 보니:
     *
     *        boss_hook 이 나온 판 **9판 중 4판** · 나오면 판당 1회
     *        **적중 4회 전부 0 (0%)** — 빗나간 이유 각도 3 · 사거리 1
     *
     *    없는 것이 아니라 **두 판에 한 번 나오고, 나오면 반드시
     *    빗나갑니다.** 설계 주석은 *"끌려온 것이 곧 반격 기회"* 라고
     *    적어 두었는데 **아무도 끌려간 적이 없습니다.**
     *
     *    이 실험대가 답할 수 있는 것은 *"이 손이 닿는가"* 까지입니다.
     *    실전 빈도는 **봇이 재고**(`npm run play`), 규칙은 여기서 잽니다.
     *
     ...
     */
    /**
     * ── ⚠️ **이 검사는 «강화 단계가 다른 판»을 한 통에 담고 있었습니다** ──
     *
     * 이 실험대는 일부러 **+0 · +2 · +5** 를 5판씩 돕니다. 강화가 오르면
     * 화력이 오르고, 구간 시간은 짧아집니다 — **그게 이 실험대의 목적**
     * 입니다. 그런데 폭은 그 15판을 **한 줄로 세워** 최대÷최소로 쟀습니다.
     *
     * 즉 *"판마다 흔들린다"* 고 부르던 값이 실은 **«+0 의 느린 판 ÷ +5 의
     * 빠른 판»** 이었습니다. 실제 숫자로:
     *
     *     2단계 — +0 의 7.3초 ÷ +5 의 1.3초 = 5.6배
     *     같은 단계 안에서만 보면 +0 1.7배 · +2 1.6배 · +5 3.7배
     *
     * 흔들림이 아니라 **강화의 효과를 재고 흔들림이라 부른 것**입니다.
     * 이 저장소의 이름표 사고 중에서도 큰 축입니다 — 「처방이 다른 둘이
     * 한 칸에 담기면 정확히 거꾸로 읽힙니다」.
     *
     * ⚠️ 그리고 이 값은 **최대÷최소**라 열다섯 판 중 **두 판**만으로
     *    정해집니다. 같은 코드로 두 번 돌려 **2.8배와 5.6배**가 나왔습니다.
     *    그러니 이 숫자의 변화를 근거로 무언가를 고쳤다고 말하면 안 됩니다
     *    (바로 앞 회차에 제가 *"5.7 → 2.8 로 줄었다"* 고 적었는데, 그건
     *    **줄어든 것이 아니라 같은 분포를 두 번 뽑은 것**이었습니다).
     *    그래서 **단계 안에서** 재고, 최대÷최소와 함께 **중앙 절반**
     *    (p75÷p25)도 같이 찍습니다 — 뒤쪽이 판 수에 덜 휘둘립니다.
     */
    const q = (a, p) => {
    ...
    const spreadIn = (rs, i) => {
      const a = rs.map((r) => r.phaseTime[i])
      return Math.max(...a) / Math.max(0.1, Math.min(...a))
    }
    const midIn = (rs, i) => {
      const a = rs.map((r) => r.phaseTime[i])
      return q(a, 0.75) / Math.max(0.1, q(a, 0.25))
    }
    /** 단계별로 재고 **가장 넓은 단계**를 씁니다 — 통을 섞지 않습니다. */
    const spread = (i) => Math.max(...[...byLevel.values()].map((rs) => spreadIn(rs, i)))
    const mid = (i) => Math.max(...[...byLevel.values()].map((rs) => midIn(rs, i)))
    const worst = Math.max(spread(0), spread(1), spread(2))
    /**
     * ── ⚠️ **이 줄은 재지 않은 범인을 지목하고 있었습니다** ────────────
     *
     * 원래 실패 문구가 이랬습니다 — *"폭의 원인이 여정이 아니라 **보스**
     * 입니다(패턴 선택)"*. 그런데 이 검사가 아는 것은 **폭이 넓다**는 것
     * 뿐입니다. 여정을 걷어냈으니 보스 쪽이라는 것까지는 맞지만, 보스의
     * 무엇인지(패턴 선택인지·전환 무적인지·플레이어 화력인지)는 **한 번도
     * 안 쟀습니다.** 이 저장소가 스스로 세운 규칙을 이 줄이 어기고
     * 있었습니다 — 「재기 전의 설명은 결론이 아니다」.
     *
     * 실제로 그 문구를 믿고 패턴 선택을 고치러 갔습니다. 절반은 맞았고
     * (같은 패턴 반복에 벌점을 넣자 폭이 **5.7배 → 2.8배**), 절반은
     * 틀렸습니다 — 남은 폭의 자리는 **2단계**인데, 그 구간은:
     *
     *     2단계 3.9(2.8~7.7)초 = **2.8배** · 화력 52.5(27.0~76.2) = **2.8배**
     *
     * 시간의 폭과 **화력의 폭이 같은 배율**입니다. 구간 시간 = 체력 ÷ 화력
     * 이니, 남은 폭은 보스가 무엇을 고르는가가 아니라 **이 손의 화력이
     * 판마다 다른 것**입니다(무방비 10.8~52.5% · 전환 무적 16.9~44.8%).
     *
     * 그래서 문구를 **범인 지목에서 «어디를 보라»로** 바꿉니다. 어느
     * 구간이 넓은지, 그 구간의 화력도 같이 넓은지를 나란히 적습니다.
     * 둘이 같이 넓으면 화력 쪽, 시간만 넓으면 보스 쪽입니다.
     */
    const dpsIn = (rs, i) => {
      const a = rs.map((r) => r.phaseDmg[i] / Math.max(0.1, r.phaseTime[i]))
      return Math.max(...a) / Math.max(0.1, Math.min(...a))
    }
    const dpsSpread = (i) => Math.max(...[...byLevel.values()].map((rs) => dpsIn(rs, i)))
    const widest = [0, 1, 2].reduce((m, i) => (spread(i) > spread(m) ? i : m), 0)
    /**
     * ── 판정을 **중앙 절반**에 겁니다 (최대÷최소는 눈금이 될 수 없습니다) ──
     *
     * 문턱을 흔들어 초록을 만드는 것이 아닙니다. 이 검사가 답하려는 질문은
     * *"이 수치로 구간 배분을 풀 수 있는가"* 이고, 배분은 **중앙값**으로
     * 풉니다. 그러면 필요한 것은 *"대다수 판이 모여 있는가"* 이지
     * *"제일 빠른 판과 제일 느린 판이 몇 배인가"* 가 아닙니다.
     *
     * 그리고 최대÷최소는 **다섯 판 중 두 판**만 보는 값이라, 같은 코드로
     * 두 번 돌려 2.8배와 5.6배가 나왔습니다. 그 값의 변화를 근거로 무언가를
     * 고쳤다고 말하는 순간 그건 측정이 아니라 도박입니다(제가 바로 앞
     * 회차에 그렇게 적었습니다 — 이 자리에서 취소합니다).
     *
     * 문턱 1.5배: 중앙 절반이 그보다 벌어지면 중앙값이 대표값 노릇을 못
     * 합니다. 지금 값은 1.2배라 **여유 0.3** — 한 칸 차이의 초록이 아닙니다.
     * 최대÷최소는 **판정에서 내리되 화면에는 남깁니다.** 못 잰 것을 통과로
     * 만들지 않되, 잰 것을 숨기지도 않습니다.
     */
    /**
     * ── ⚠️ **검사를 내리고 «기록»으로 둡니다** ────────────────────────
     *
     * 판정을 중앙 절반으로 옮겨 봤더니 이번엔 그것도 흔들렸습니다 —
     * 같은 코드로 세 번 돌려 **1.2 · 1.5 · 2.5배**(문턱으로 잡으려던 값이
     * 1.5였습니다). 다섯 판에서 p25·p75 는 **2번째와 4번째 판**이라,
     * 판 하나가 자리를 바꾸면 값이 그만큼 움직입니다.
     *
     * 그러면 이 줄은 **문턱이 아니라 동전**입니다. 흔들리는 검사는 없는
     * 것보다 나쁩니다 — 빨간불을 무시하는 습관을 만듭니다(이 세션에서만
     * 그렇게 방치된 빨강을 셋 찾았습니다).
     *
     * 그래서 **기록**으로 둡니다. 이 저장소에 같은 판단의 선례가 있습니다
     * (🔁 「되돌아오는 고리」 줄 — *"검사를 하나 썼다가 관찰로 내렸습니다"*).
     * 다시 검사로 올릴 조건도 적어 둡니다: **단계마다 판 수를 늘려야**
     * 합니다(다섯 판으로는 이 통계가 안 섭니다). 지금 이 프로브는 15판에
     * 20분이 걸리므로, 그건 판을 짧게 만드는 일과 같이 와야 합니다.
     *
     * 대신 **흔들리지 않는 것**을 같이 적습니다. 세 판에서 한결같았던 것:
     *   · 1·3단계의 중앙 절반은 **1.0~1.1배** — 거의 안 흔들립니다
     *   · 흔들리는 자리는 늘 **2단계**이고, 그 구간은 셋 중 가장 짧으며
     *     (10초 · 4~6초 · 7~9초) 그 시간의 **3분의 1이 전환 무적**입니다
     * 즉 「폭」의 정체는 *"가운데 단계가 너무 짧다"* 는 배분 이야기입니다.
     */
    console.log(
      `  [구간 폭] 같은 강화 단계 안에서 — ` +
        [0, 1, 2]
          .map((i) => `${i + 1}단계 중앙절반 ${mid(i).toFixed(1)}배(최대÷최소 ${spread(i).toFixed(1)})`)
          .join(' · '),
    )
    console.log(
      `            가장 넓은 곳은 **${widest + 1}단계** · 그 구간의 화력 ${dpsSpread(widest).toFixed(1)}배` +
        (dpsSpread(widest) >= spread(widest) * 0.8
          ? ' → 둘이 같이 넓습니다(**화력** — 무방비·전환 무적·처형)'
          : ' → 시간만 넓습니다(**보스가 고르는 것**)') +
        `\n            ⚠️ 판정 아님 — 다섯 판으로는 이 통계가 안 섭니다(같은 코드로 1.2 · 1.5 · 2.5배).`,
    )
    /**
