...
for (const f of files) {
  ...
  for (const b of evaluateBlocks(src)) {
     ...
     */
    /**
     * ⚠️ **꼬리가 배열이 아니라 «홑이름»일 때도 잘라야 합니다 (세 번째).**
     *
     * 이 정규식은 `, [A, B])` 만 잘랐습니다. 그런데
     * `page.evaluate(fn, PRESSURE_FULL)` 처럼 인자 하나를 **그대로** 넘기면
     * 못 잘라서, **제대로 넘긴 인자를 «몸통에서 참조한 것»** 으로 읽고
     * 빨강을 냈습니다.
     *
     * 그리고 그 빨강 앞에서 저는 *"이름이 겹쳐서 그런가 보다"* 하고
     * **넘겨짚어 이름만 바꿨습니다.** 그 순간 검사는 초록이 됐는데,
     * 인자는 여전히 안 넘어가고 있었습니다 — 안에서 `undefined` 가 되고
     * falsy 라 기본 경로만 «우연히» 맞게 돌던 상태였습니다.
     *
     * ⭐ **초록이 됐다고 고쳐진 게 아닙니다.** 빨강의 «이유»를 안 읽고
     *    빨강만 없애면, 검사는 통과하고 버그는 남습니다. 이 파일 위쪽이
     *    같은 자리에서 두 번 적어 둔 문장 — *"규칙은 맞았는데 읽는 법이
     *    무르다"* — 의 세 번째 사례입니다.
     */
    body = body.replace(
