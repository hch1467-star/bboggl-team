...
const TARGETS = [
  ...
  { id: 'elite', label: '정예  ' },
  /**
   * 🛡 **철갑병** — 이 적을 넣으려고 만든 적입니다. 세 판에 걸쳐 「간파는
   *    보스에서만 값어치가 난다」가 재현됐고, 원인이 «모든 비보스를 그냥
   *    때려서 이길 수 있다» 였습니다. 철갑병은 그 선택지를 지웁니다
   *    (`armor: 0.35`). **그러면 간파가 여기서도 이기는지**가 이 줄의 질문
   *    입니다 — 안 이기면 이 적은 답을 좁힌 게 아니라 **그냥 벽**입니다.
   */
