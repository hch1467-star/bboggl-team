...
const TARGETS = [
   ...
   * 🛡 **철갑병.**
   *
   * ⚠️ **이 계기로는 이 적의 값어치를 못 잽니다 — 알고 넣습니다.**
   *
   * 첫 판이 이렇게 나왔습니다: 「철갑 · 간파노림 0.169/초 vs 평타만
   * 0.243/초 — **평타가 이깁니다**」. 저는 이걸 *"철갑병은 답을 좁힌 게
   * 아니라 그냥 벽이다"* 로 읽을 뻔했습니다.
   *
   * **틀린 읽기입니다.** 이 계기는 «**무너뜨리는** 속도»를 재는데, 철갑의
   * 전부는 `armor: 0.35` — **«죽이는** 속도»에 붙습니다. 게다가 이 계기는
   * 상대의 체력을 **매 프레임 가득 채웁니다**(재려는 것이 강인도지 승패가
   * 아니므로). 그러니 이 판에서 철갑은 그냥 «강인도 95 짜리 정예»이고,
   * **갑옷은 한 번도 작동하지 않았습니다.**
   *
   * ⭐ **재는 축이 틀리면 숫자는 답처럼 생긴 채로 나옵니다.** 이 회차가
   *    여러 번 겪은 그 모양이고, 이번에는 «답이 마음에 안 드는» 쪽으로
   *    나와서 다행히 한 번 더 봤습니다.
   *
   * 여기 남겨 두는 이유: 강인도 축에서 철갑이 정예·보스와 **어디쯤인지**는
   * 이 계기가 정확히 잽니다. 값어치는 **`npm run bench`(죽이는 축)** 가
   * 답합니다.
   */
  { id: 'ironclad', label: '철갑  ' },
  { id: 'boss', label: '보스  ' },
]
/**
...
try {
  ...
  /**
   * ── ⚔️ **죽이는 축 — 갑옷은 여기서만 보입니다** ─────────────────────
   *
   * ── 왜 따로 재는가 (제가 한 번 잘못 읽었습니다) ─────────────────────
   * 위의 모든 줄은 «**무너뜨리는** 속도»를 잽니다. 그런데 철갑병의 전부는
   * `armor: 0.35` — «**죽이는** 속도»에 붙습니다. 게다가 위 판들은 상대의
   * 체력을 **매 프레임 가득 채웁니다**(재려는 것이 강인도지 승패가
   * 아니므로). 그래서 위 판에서 철갑은 그냥 «강인도 95 짜리 정예»이고
   * **갑옷은 한 번도 작동하지 않습니다.**
   *
   * 첫 판에서 「철갑 · 간파노림이 평타에게 진다」를 보고 저는
   * *"이 적은 답을 좁힌 게 아니라 그냥 벽이다"* 로 읽을 뻔했습니다.
   * **재는 축이 틀리면 숫자는 답처럼 생긴 채로 나옵니다.**
   *
   * ── 여기서 묻는 것 ────────────────────────────────────────────────
   * **연타로 죽이는 시간 vs 무너뜨리고 죽이는 시간.**
   *
   *   · 보통 적이면 둘이 비슷해야 합니다 — 갑옷이 없으니까요.
   *   · 철갑이면 **무너뜨리는 쪽이 뚜렷이 빨라야** 합니다. 안 그러면
   *     `armor` 는 답을 좁힌 게 아니라 **그냥 체력을 늘린 것**입니다.
   *
   * ⚠️ 체력을 **안 채웁니다.** 그것이 이 절의 전부입니다.
   */
  console.log(`\n⚔️  죽이는 축 — 갑옷이 실제로 답을 좁히는가 (체력을 안 채웁니다)\n`)
  const killRows = []
  for (const tgt of TARGETS) {
    for (const hand of [0, 1]) {
      const r = await page.evaluate(
        async ([kindId, hand, capSeconds]) => {
          const G = window.__game
          const now = () => G.state().simElapsed
          G.clearEnemies()
          await window.__t.runFor(0.2)
          const foe = G.spawnEnemyKind(kindId, 8, 0)
          if (foe < 0) return { error: `${kindId} 을(를) 못 세웠습니다` }
          await window.__t.runFor(0.3)
          const t0 = now()
          let guard = 0
          ...
          const fi = G.enemyInfo(foe)
          const dead = !fi || fi.hp <= 0
          const took = now() - t0
          G.clearEnemies()
          return { dead, took: Number(took.toFixed(1)), broke }
        },
        [tgt.id, hand, CAP_SECONDS],
      )
      if (r.error) {
        console.log(`  💥 ${r.error}`)
        continue
      }
      killRows.push({ target: tgt.id, label: tgt.label, hand, ...r })
    }
  }
  /**
   * ⚠️ **분모부터.** 시간 상한에 걸려 «안 죽은» 줄로 비율을 만들면, 그건
   *    «오래 걸렸다»가 아니라 **«못 쟀다»** 입니다.
   */
  const measured = killRows.filter((r) => r.dead)
  check(
    measured.length === killRows.length && killRows.length === TARGETS.length * 2,
    '⚔️ 모든 상대를 두 손 다 **실제로 죽였다** (판정의 게이트)',
    `${measured.length}/${TARGETS.length * 2}` +
      (measured.length < killRows.length
        ? ` — 못 죽인 것: ${killRows.filter((r) => !r.dead).map((r) => `${r.label.trim()}(손${r.hand})`).join(' · ')}`
        : ''),
  )
  for (const tgt of TARGETS) {
    const a = killRows.find((r) => r.target === tgt.id && r.hand === 0)
    const b = killRows.find((r) => r.target === tgt.id && r.hand === 1)
    if (!a || !b || !a.dead || !b.dead) continue
    const faster = a.took / b.took
    console.log(
      `  ${tgt.label}연타만 ${a.took}초 (붕괴 ${a.broke}) vs 무너뜨리고 ${b.took}초 (붕괴 ${b.broke}) — ` +
        (faster >= 1.25
          ? `**무너뜨리는 쪽이 ${faster.toFixed(2)}배 빠릅니다**`
          : faster <= 0.8
            ? `연타가 ${(1 / faster).toFixed(2)}배 빠릅니다`
            : '비슷합니다'),
    )
  }
  /**
   * 🛡 **이 적을 만든 이유가 참인가.** 철갑에서 무너뜨리는 쪽이 뚜렷이
   *    빠르지 않으면 `armor` 는 답을 좁힌 것이 아니라 **체력을 늘린 것**
   *    입니다 — 그건 이 저장소가 「벽」이라 부르는 것입니다.
   */
  const ia = killRows.find((r) => r.target === 'ironclad' && r.hand === 0)
  const ib = killRows.find((r) => r.target === 'ironclad' && r.hand === 1)
  if (ia?.dead && ib?.dead) {
    check(
      ia.took / ib.took >= 1.25,
      '🛡 **철갑은 무너뜨리는 쪽이 뚜렷이 빠르다** (아니면 갑옷은 답이 아니라 벽입니다)',
      `연타 ${ia.took}초 vs 무너뜨리고 ${ib.took}초 — ${(ia.took / ib.took).toFixed(2)}배`,
    )
  }
