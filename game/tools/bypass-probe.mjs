...
try {
  ...
  /**
   * **추격이 장식인가.** 아무도 사거리에 못 닿으면 "따라온다"는 말은
   * 화면 뒤편의 행렬일 뿐입니다. 달리는 플레이어가 오는 길에 한 대도
   * 안 맞으면 이 존의 전투는 전부 선택 사항입니다.
   */
  check(
    run.hits >= 1,
    '달려서 지나가는 동안 최소 한 번은 닿는다 (추격이 장식이 아니다)',
    `피격 ${run.hits}회(${run.hitsSpan}) · 걸어서는 ${walk.hits}회(${walk.hitsSpan})`,
  )
