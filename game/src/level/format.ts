 ...
 * ── 🪜 **되돌아올 수 없는 길의 값** (칸 단위) ─────────────────────────
 *
 * 길찾기가 한 칸 옮기는 값은 1 입니다. 그런데 **뛰어내리면 다시 못 오르는**
 * 칸으로 옮기는 것은 값이 달라야 합니다. 지금까지는 같았고, 그래서
 * 길안내가 *"여기서 뛰어내리면 3m 이득"* 이라는 이유로 **되돌아올 수 없는
 * 구덩이를 권할 수 있었습니다.**
 *
 * ── 실측이 이걸 어떻게 드러냈는가 ────────────────────────────────
 * 이 존에는 보스로 가는 길이 둘입니다(`npm run route`):
 *
 *     북쪽 계단(안전)        194m
 *     남쪽 일방통행 고리      206m   ← 겨우 **12m** 차이
 *
 * 12m 는 지형을 한 칸만 손대도 뒤집히는 차이라, **동선이 판마다
 * 흔들렸습니다.** 그 흔들림이 보물 시야·곁길 알림 측정을 통째로
 * 깜빡이게 만들고 있었습니다(4/4 ↔ 3/4). 즉 이 값이 없으면 "무엇이
 * 주 동선인가"조차 재현되지 않습니다.
 *
 * ── 다른 게임은 어떻게 하는가 ───────────────────────────────────
 * 내비메시를 쓰는 게임들(Recast/Detour 계열)은 **점프 링크에 따로 비용**을
 * 매깁니다 — 링크가 있다는 것과 그 길을 권한다는 것은 다른 문제이기
 * 때문입니다. 소울류는 화살표가 없어서 이 문제를 안 겪고, 로스트아크에는
 * 되돌아올 수 없는 길 자체가 없습니다. 화살표가 있으면서 일방통행 지형을
 * 가진 우리는 **값을 매기는 쪽**을 택합니다.
 *
 * ── 11칸(22m)의 근거 ────────────────────────────────────────────
 * 카메라가 세로로 담는 거리가 22m 입니다(CAMERA.viewSize). 그래서 이
 * 값은 사람이 읽을 수 있는 한 문장이 됩니다:
 *
 *     **"화면 한 장 넘게 아끼는 게 아니면, 되돌아올 수 없는 길은 안 권한다."**
 *
 * ⚠️ 이건 **안내와 판단의 값**이지 통행 금지가 아닙니다. 플레이어는
 *    언제든 뛰어내릴 수 있고, 실제로 그게 더 빠르면(22m 넘게 아끼면)
 *    안내도 그쪽을 가리킵니다. 막는 것이 아니라 **기본값을 정하는** 것입니다.
 */
export const ONE_WAY_COST = 11
