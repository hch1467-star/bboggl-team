...
/**
 * 🏛 잔해 지물의 밝기 — **바닥 잔해와 기둥을 반대로** 갑니다.
 *
 * 하나의 배율로 둘 다 맞추려다 화면을 다섯 번 다시 찍었습니다. 1.1 이면
 * 기둥이 벽에 묻히고, 1.35 면 바닥 돌이 조명처럼 떠서 어두운 톤을 깼습니다.
 * 둘이 **서 있는 배경이 반대**라서 그렇습니다:
 *
 *   · 바닥 잔해는 **밝은 바닥 위**에 있습니다 → 어두워야 돌로 보입니다
 *   · 기둥은 **허공(검정) 곁**에 섭니다      → 밝아야 실루엣이 보입니다
 *
 * 배경 대비로 읽히게 하는 것이 밝기를 하나로 맞추는 것보다 훨씬 눈에 잘
 * 듭니다 — 4색 예고를 "색이 안 보여도 크기로 읽히게" 만든 것과 같은 이유로,
 * 여기서도 가장 튼튼한 채널인 **대비**를 씁니다.
 */
const RUBBLE_MUL = 0.72
const PILLAR_MUL = 1.34
/**
...
export class Terrain {
   ...
   */
  /**
   * 🏛 **잔해 지물의 색** — 그 칸의 윗면 색을 그대로 받아 밝힙니다.
   *
   * 회색 돌을 그냥 쓰면 *"밝기는 높이가, 색온도는 구역이 말한다"* 는 이
   * 파일의 규칙을 지물만 어깁니다 — 구역이 바뀌어도 잔해만 같은 색으로
   * 남아 화면이 두 겹으로 칠해진 것처럼 보입니다.
   *
   * 밝히는 이유는 하나입니다: **벽과 같은 색이면 벽의 일부로 보입니다.**
   * 지물은 벽 위·벼랑 턱처럼 어두운 면 곁에 서므로, 밝기로 떼어 놓지
   * 않으면 세워 놓고도 안 보입니다.
   *
   * ⚠️ **더하기로 올렸다가 되돌렸습니다.** 첫 화면에서 잔해가 거의 흰색으로
   *    떠서 폐허가 아니라 **포석**처럼 보였습니다. 더하기는 어두운 채널을
   *    비율로 크게 밀어 올려 색조를 지웁니다(0.10 → 0.15 는 50% 증가입니다).
   *    곱하기는 채널 비율을 지키므로 구역 색이 살아남고, 높이 램프의 순서도
   *    그대로입니다 — 모든 높이에 같은 배율이 걸리니까요.
   */
  propColor(lvl: number, cx: number, cz: number, tall: boolean): [number, number, number] {
    const ti = (cz * this.level.w + cx) * 3
    const tint: [number, number, number] = [this.tintAt[ti], this.tintAt[ti + 1], this.tintAt[ti + 2]]
    const [r, g, b] = this.topColor(lvl, 1, tint)
    const k = tall ? PILLAR_MUL : RUBBLE_MUL
    return [r * k, g * k, b * k]
  }
