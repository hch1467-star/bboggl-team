...
class Game {
  ...
  private frame(nowMs: number): void {
    ...
    for (const b of breakEvents) {
       ...
       *
       * ⚠️ 이 한 줄이 **처음에 빠져 있었습니다.** 선언(`breakBy`)도,
       *    내보내기(`runStats`)도, 읽는 쪽(벤치)도 다 있는데 **세는 곳만**
       *    없어서 벤치가 「붕괴가 한 번도 없었습니다」를 찍었습니다 —
       *    같은 판의 다른 줄은 붕괴 3회를 찍고 있는데도요.
       *
       *    `lungeSpeed` 와 **정확히 같은 모양**입니다: 칸은 있고 값을 넣는
       *    곳이 없어 조용히 0으로 읽히는 것. 다행히 「빈 표본으로 통과하지
       *    않게」 게이트가 0으로 나누지 않고 멈춰서 **초록으로 위장되지는
       *    않았습니다.**
       */
      this.breakBy[b.by ?? '평타'] = (this.breakBy[b.by ?? '평타'] ?? 0) + 1
      /**
       * **무너진 순간의 체력**을 기록합니다.
       *
