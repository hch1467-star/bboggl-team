...
export function bossTakenScale(t: number): number {
  /**
   * ── 🛡 **철갑 — 무너뜨리기 전에는 잘 안 통합니다** ────────────────────
   *
   * ── 왜 이 자리를 빌리는가 ────────────────────────────────────────
   * 「받는 피해 배율」의 규칙은 **여기 한 곳에** 있습니다(체력·출혈 둘 다
   * 이 함수를 지나갑니다). 철갑을 위해 두 번째 배율 자리를 만들면, 언젠가
   * 한쪽만 고쳐집니다 — 이 저장소가 이번 회차에만 다섯 번 겪은 모양입니다.
   * 이름이 `boss…` 인 것은 역사일 뿐이고, **하는 일은 «이 적이 지금 받는
   * 피해 배율»** 입니다.
   *
   * ── 왜 이 적이 필요한가 (잰 것에서 나왔습니다) ────────────────────
   * 이번 회차가 세 판에 걸쳐 재현한 것: **간파는 보스에서만 값어치가
   * 납니다**(보스 +13~14%, 잡몹·정예는 비기거나 짐). 그리고 게임에서
   * **보스가 유일한 간파 상대**입니다.
   *
   * 원인은 «간파할 예고가 없어서»가 아닙니다 — `npm run poise` 가 적 공격
   * **15/15 전부 간파 가능**이라고 찍었습니다. 원인은 다른 것입니다:
   *
   *     **모든 비보스는 «그냥 때려서» 이길 수 있습니다.**
   *
   * 무너뜨리는 것이 «더 빨리 이기는 길»일 뿐이면, 더 쉬운 길(연타)이
   * 언제나 이깁니다. 이 회차가 예고중 배수에서 배운 그것과 같은 모양
   * 입니다 — **배수는 물량으로 이깁니다.**
   *
   * 그래서 이 적은 **선택지를 지웁니다.** 무너지기 전에는 피해가 크게
   * 줄어서, 연타로는 못 이깁니다. 새로 외울 색도 패턴도 없습니다(잡몹과
   * **같은 공격 목록**을 씁니다 — 정예와 같은 계약). 새것은 하나뿐입니다:
   * *"이놈은 무너뜨려야 한다."*
   *
   * 참고 게임의 같은 자리 — 소울류의 철갑 기사, 오공의 갑주를 두른 적,
   * 로스트아크의 무력화 구간. 셋 다 **답을 하나로 좁히는** 적입니다.
   */
  const armor = enemyDef(Enemy.kind[t]).armor
  if (armor !== undefined && armor < 1 && Enemy.brokenT[t] <= 0) return armor
...
function applyPoise(t: number, spec: AttackSpec, behind = false, crossfire = false): void {
  ...
  /**
   * ── 🎯 **간파 — 배수를 버리고 «사건»으로 바꿉니다** ────────────────────
   *
   * ── 왜 배수로는 안 되는가 (재고 나서 바꿨습니다) ──────────────────
   * `POISE.windupMultiplier`(×2.5)를 예고의 마지막 구간에만 걸게 하고
   * `npm run heavy` 로 **창만 노리는 손**을 만들어 재 봤습니다:
   *
   *     초당 붕괴 — 그냥 연타 **0.200**  vs  창만 노림 **0.084**
   *     보스에서는 70초에 붕괴 **1회** (창에서 320번 휘둘렀는데도)
   *
   * 못 맞혀서가 아닙니다 — 롱소드 1타의 준비는 0.12초라 0.3초 창 안에
   * 넉넉히 들어갑니다. 원인은 **강인도 회복**이었습니다:
   *
   *     POISE.regenDelay 2.2초 · regenPerSec 18
   *
   * 창은 2~3초에 한 번 0.3초만 열립니다. 창만 노리는 손은 한 번 치고
   * **회복 지연을 넘겨서 기다리므로**, 깎은 것을 도로 돌려줍니다.
   *
   * ⭐ **배수는 물량으로 이깁니다.** 배수를 아무리 키워도, 「많이 치는 손」이
   *    「잘 치는 손」을 이깁니다 — 배수는 **타수에 비례**하기 때문입니다.
   *    판단을 사고 싶으면 **타수에 비례하지 않는 것**을 줘야 합니다.
   *
   * 이 저장소의 장부가 이미 그 답을 갖고 있었습니다:
   *
   ...
   *
   * **사건인 지렛대는 일하고, 배수인 지렛대는 안 합니다.**
   *
   * ── 왜 «강타로만» 인가 ────────────────────────────────────────────
   * 아무 평타나 창에 걸리면 그냥 연타가 다시 이깁니다(연타는 이미 창에
   * 11%가 저절로 들어갑니다). 그래서 **전용 입력**을 요구합니다 —
   * 참고 게임이 전부 그렇게 합니다:
   *
   *     오공   「識破」  — 지정된 초식 + 정확한 순간
   ...
   *
   * 강타를 고른 이유가 셋입니다:
   *   ① **집중**을 태웁니다 — 자원이 빈도의 상한을 대신 만들어 줍니다
   *   ② 설계가 «끊는 수단»으로 이름 댄 셋 중 가장 안 쓰이던 것입니다(4~9%)
   *   ③ 우클릭은 이미 «태우는» 손이라, 새 키를 안 외워도 됩니다
   *
   * ⚠️ 보상이 큽니다(강인도를 얼마가 남았든 통째로 부숩니다). 대신
   *    **읽고 · 자원을 모으고 · 0.3초 안에 넣어야** 나옵니다. 이 게임이
...
export type BreakCause =
  ...
  | '예고중'
  /**
   * 🎯 **간파** — 예고의 창에서 «강타»로 맞힌 것. 배수가 아니라 **사건**입니다.
   *    아래 `applyPoise` 의 간파 처리에 왜 배수를 버렸는지 적어 뒀습니다.
   */
  | '간파'
