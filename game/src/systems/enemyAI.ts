...
export function enemyAiSystem(
  ...
  for (let i = 0; i < enemies.count; i++) {
    ...
    if (tokens.has(e) && Actor.cooldownT[e] <= 0 && facingError <= ATTACK_FACING_TOLERANCE) {
       ...
       * 0.5 배면 반복이 21% → **10%** 로 줄고, 여전히 가끔은 두 번 옵니다.
       *
       * ⚠️ 값은 0.3 이 아니라 **0.5** 입니다. 0.3 은 반복을 같은 만큼
       *    줄이면서 **보스의 휘두름 간격을 3.6→4.2초로 늦췄습니다**
       *    (패턴마다 한 바퀴 길이가 달라서 — 자세한 실측은
       *    `enemyAttacks.ts` 의 `REPEAT_PENALTY` 자리).
       *
       * ── 왜 여기(가중치 덮어쓰기)에 넣는가 ────────────────────────────
       ...
       */
      /**
       * ── ⚠️ **«잡몹에게는 빼자» 를 넣었다가 도로 뺐습니다** ───────────────
       *
       * `npm run bypass` 에 오래된 빨강이 있었습니다:
       *
       *     ❌ 끌고 온 무리가 실제로 아프다 — 8초 청구서 **16(16~28)**
       *        (그 검사의 옛 기록은 32~79 · 기준은 성수병 한 병 45)
       *
       * 그럴듯한 설명이 있었습니다 — 잡몹의 두 패턴은 예고가 두 배 차이라
       * (jab 0.63초 · sweep 1.25초), 벌점이 굴림을 긴 쪽으로 밀면 8초에
       * 들어가는 휘두름이 줄어든다. 보스에게서 실제로 그 일이 있었으니
       * (3.6→4.2초) 같은 기제로 보였습니다.
       *
       * **그래서 벌점을 보스에게만 걸고 다시 쟀습니다 — 값이 안 움직였습니다.**
       *
       *     모두에게 걸었을 때  16(16~28) · 16(16~41) · 28(16~41)
       *     보스에게만 걸었을 때 16(16~28) · 16(16~28)
       *
       * 설명이 틀렸습니다. 그러면 그 설명 위에 세운 특례도 **근거가
       * 없습니다.** 이 저장소의 규칙대로 되돌립니다 — 「재기 전의 설명은
       * 결론이 아니다」는 **되돌릴 때에도** 같습니다. 특례를 남겨 두면
       * 다음 사람은 그것이 측정으로 뒷받침된 줄 압니다.
       *
       * 청구서가 낮은 진짜 이유는 아직 모릅니다. 다음 후보는 **동시 공격
       * 토큰**입니다 — 8초 안에 몇 번이나 휘두를 수 있는지의 천장을 먼저
       * 재야 합니다(`bypass` 프로브 안에 적어 뒀습니다).
       */
