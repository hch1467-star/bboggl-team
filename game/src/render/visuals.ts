...
export class Visuals {
  ...
  private attachCrackedWall(entity: number, group: THREE.Group, tough = false): void {
     ...
     *
     * ── 그림이 잡은 것 ────────────────────────────────────────────────
     * 두꺼운 벽을 동선 옆으로 옮기고 `npm run hide` 로 찍어 봤더니,
     * **1.5m 코앞에서도 벽이 안 보였습니다.** 금 간 벽은 같은 거리에서
     * 누런 판이 또렷하게 서 있는데 말입니다.
     *
     * 이유는 색 셋이 전부 어두웠기 때문입니다:
     *     지형 바위  어두운 청회색
     *     두꺼운 벽  0x6d7482  (어두운 청회색 — **바위와 같은 계열**)
     *     금        0x2b2118  (거의 검정 — **벽 위에서도 안 보임**)
     *
     * 위 ②의 *"더 어둡고 푸른 돌"* 은 **금 간 벽과 견주어** 정한 값입니다.
     * 그때 **지형과는 안 견줬습니다.** 두 물건을 가르는 데만 신경 쓰다가
     * 배경에 묻힌 것이라, 이 저장소가 4색 예고에서 겪은 것과 같은 실패입니다
     * (`npm run contrast` 가 예고 색을 **바닥과** 견주는 이유).
     *
     * ── 그래서 색이 아니라 **금**을 고칩니다 ──────────────────────────
     * 벽 색을 밝히면 위 ②(두 벽이 다른 계열)가 무너지고, 붉게 하면
     * *"적은 전부 붉은 계열"* 과 부딪힙니다. 대신 **단서 자체**를 밝힙니다:
     *
     *     **금은 언제나 벽과 반대 밝기다.**
     *       · 누런(밝은) 벽 → 어두운 금
     *       · 청회색(어두운) 벽 → **밝은 금**
     *
     * 이게 의미로도 맞습니다 — 플레이어가 알아봐야 하는 것은 돌이 아니라
     * **갈라진 자리**입니다. `MeshBasicMaterial` 이라 빛과 무관하게 같은
     * 밝기로 나오므로, 그늘에 선 벽에서도 금은 그대로 읽힙니다.
     *
     * ⚠️ 벽 자체는 여전히 **안 빛납니다**(emissive 0). 위 ⚠️ 의 약속
     *    (*"둘 다 빛나면 둘은 같은 것이라고 말하게 된다"*)은 그대로입니다 —
     *    밝아진 것은 벽이 아니라 금 한 줄입니다.
     */
    const crackMat = new THREE.MeshBasicMaterial({ color: tough ? 0xd8cfc0 : 0x2b2118 })
