...
export const CHARGER_ATTACKS: EnemyAttackDef[] = [
  {
    ...
    weight: 1,
    /**
     * ── 11 m/s 는 어디서 나온 숫자인가 ──────────────────────────
     * 필요한 것은 "빠른 느낌"이 아니라 **닿는 것**입니다. 그래서 거꾸로 풀었습니다:
     *
     *   · 예고가 시작될 수 있는 가장 먼 거리 = `maxRange` 7.5m
     *   · 예고 1.4초 동안 달리는 플레이어가 버는 거리 = 8.4 × 1.4 = 11.8m
     *   · 판정이 뜨는 순간 닿아야 하는 거리 = `reach` 5.5m
     *   → 1.4초에 7.5 + 11.8 − 5.5 = **13.8m** 를 좁혀야 합니다 = 9.9 m/s
     *
     * 9.9는 **딱 맞는 값**이라 지형·회전·가속에서 조금만 새도 못 닿습니다.
     * 11 로 두면 여유가 1.5m 남습니다. 반대로 이보다 크게 올리면
     * "옆으로 꺾어서 피한다"는 답이 사라집니다 — 꺾을 시간이 없어집니다.
     *
     * 위 세 숫자(7.5 · 8.4 · 5.5) 중 하나라도 바뀌면 이 값도 다시 풀어야 합니다.
     * `npm run bypass` 의 "달려서 지나가는 동안 최소 한 번은 닿는다" 가 지킵니다.
     */
    lungeSpeed: 11,
  },
]
