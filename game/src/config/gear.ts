 ...
 * 둘이고 각 여섯이니 **열두 개가 전부**입니다 — 그래서 무엇을 살지가
 * 진짜 선택이 됩니다.
 *
 * ── 값은 **새로 안 만듭니다** ─────────────────────────────────────
...
export const SHOP = {
  /**
   * 한 모루가 내놓는 물건 수 — **무기 종류마다 둘씩**.
   *
   * ── 왜 무작위로 안 뽑는가 ────────────────────────────────────────
   * 무작위로 여섯을 뽑으면 *"내가 안 쓰는 무기 것만 여섯"* 이 나올 수
   * 있고, 그러면 그 상점은 그 판에 없는 것과 같습니다. 종류마다 고정으로
   * 내면 상점은 늘 관련이 있고, 덤으로 *"다른 무기에 투자한다"* 는
   * 선택지가 눈에 보이게 됩니다.
   *
   * ── ⚠️ **하나였다가 둘로 늘렸습니다** ───────────────────────────
   * 하나면 *"어느 무기에 투자할까"* 는 선택이 되지만, **한 무기를 밀고
   * 있는 사람에게는 선택이 아닙니다** — 모루가 내놓는 것이 딱 하나라
   * 살지 말지밖에 없습니다. 둘이면 같은 무기 안에서도 고르게 됩니다:
   * 보통 **싼 것 하나와 비싼 것 하나**, 또는 같은 등급의 **다른 옵션**
   * (공격력이냐 쿨타임이냐)이 나란히 섭니다.
   *
   * 재입고가 없다는 약속은 그대로입니다. 모루 둘 × 여섯 = **열둘이
   * 전부**이고, 한 판 벌이로는 그중 두어 개밖에 못 삽니다 — 늘어난 것은
   * 살 수 있는 양이 아니라 **고를 수 있는 폭**입니다.
   */
  perWeapon: 2,
  /**
