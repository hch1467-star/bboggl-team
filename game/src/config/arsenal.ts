 ...
 */
/**
 * 📏 **이 타가 실제로 닿는 반지름**(m) — 화면에 그려야 할 값.
 *
 * ── 왜 `range` 가 아닌가 (재고 나서) ────────────────────────────────
 * `visuals.ts` 는 내 공격 범위를 `step.range` 로 그리면서 주석에
 * *"'예고는 좁은데 실제로는 넓다' 같은 거짓말이 생기지 않습니다"* 라고
 * 적어 두었습니다. 재 보니 사실이 아니었습니다(`npm run reach`,
 * 적을 고정하고 거리를 훑음):
 *
 *     그린 선 **2.3m** · 실제 명중 끝 **3.3m** (3.5m 부터 빗나감)
 *     1.9 ~ 3.3m 전부 명중 — 그린 선 밖 5곳
 *
 * 1.0m 차이입니다. 둘로 갈립니다:
 *   · **파고들기(lunge)** — 공격 시작에 내가 앞으로 미끄러집니다. 내 몫.
 *   · 대상의 굵기 — `range + Body.radius[대상]`(combat.ts). 상대 몫.
 *
 * 여기서는 **내 몫만** 더합니다. 대상의 굵기는 적마다 다르고, 그리는
 * 시점에는 누구를 칠지 모릅니다. 모르는 것을 그리면 그건 또 다른
 * 거짓말입니다 — 남은 굵기 몫은 **알고 남기는 여유**로 둡니다.
 *
 * ── ❌ **그런데 이 함수로 그렸다가 되돌렸습니다** ──────────────────
 * `range + lunge` 로 그렸더니 **3.8m** 이 나왔습니다. 그런데 실제 명중
 * 끝은 **3.3m** 입니다. 파고들기가 **적응형**(멀면 더 미끄러지고 가까우면
 * 안 미끄러짐)이라 `lunge` 는 실제 이동이 아니라 **상한**이었습니다.
 *
 * 즉 이 값으로 그리면 **없는 사거리를 약속합니다.** 좁게 그리는 거짓말은
 * "필요 이상으로 붙게" 만들지만, 넓게 그리는 거짓말은 **헛치게** 만듭니다.
 * 뒤쪽이 더 나쁩니다 — 헛친 뒤딜은 이 게임에서 가장 비싼 실수입니다.
 *
 * 그래서 그림은 `step.range` 로 되돌렸고, 이 함수는 **재는 쪽이 상한을
 * 물어보는 용도**로만 남깁니다. 목표(대상 굵기까지 반영한 참값)를 그리려면
 * 적응형 파고들기의 실제 이동량을 게임이 내보내야 합니다 — 그건 다음 일감.
 */
export function swingRadiusUpperBound(step: { range: number; lunge: number }): number {
  return step.range + step.lunge
}

/**
 * (아래는 되돌리기 전의 이름 — 쓰는 곳이 없어졌습니다.)
 *
 * ── 왜 굳이 넓히는가 (내게 유리한 거짓말인데) ────────────────────────
 * 적 예고가 좁게 그려졌을 때는 *"안전한 줄 알았는데 맞는"* 손해였고,
 * 이건 반대로 이득처럼 보입니다. 그런데 해악의 모양은 같습니다 —
 * **필요 이상으로 붙으라고 말합니다.** 등 뒤 표시가 좁아서 백어택을
 * 20% 밖에 못 쓰던 것과 같은 이야기입니다. 붙는 것은 이 게임에서
 * 가장 비싼 행동입니다.
 */
...
/**
 * ── 🌀 **이 단은 어느 쪽으로 지나가는가** (+1 = 좌→우 · −1 = 우→좌) ────
 *
 * ── 무엇이 문제였나 ──────────────────────────────────────────────
 * 지금까지 **모든 단이 같은 방향**으로 지나갔습니다. 각도와 길이만 달랐죠.
 * 그래서 3연타가 화면에서는 *"같은 휘두름을 세 번"* 으로 보였습니다.
 * 손끝은 이미 갈라 놨는데(히트스톱 0.055 → 0.11) **눈은 여전히
 * 한 덩어리**였던 셈입니다.
 *
 * ── 왜 번갈아야 하는가 — 멋이 아니라 읽기입니다 ─────────────────────
 * 참고한 게임은 전부 방향을 번갈아 줍니다:
 *
 *   · 오공 — 봉을 좌우로 번갈아 훑다가 마지막에 위에서 내려찍습니다
 *   · 위키드 — 도끼가 한 번 가로, 한 번 되돌아오며 가로
 *   · 로스트아크 — 평타 사슬이 방향으로 단을 셉니다
 *
 * 이유는 같습니다: 방향이 바뀌면 **"다음 단이 나갔다"가 눈에 보입니다.**
 * 콤보 창을 놓쳤는지 이어졌는지를 숫자나 UI 없이 화면만 보고 압니다.
 * 이 게임은 콤보 창이 0.36~0.55초로 짧아서, 그 신호가 특히 필요합니다.
 *
 * ── 왜 짝·홀로만 정하는가 ────────────────────────────────────────
 * 단마다 방향을 손으로 적어 둘 수도 있었지만, 그러면 무기를 하나 늘릴 때마다
 * **또 적어야 할 것**이 하나 늘고, 안 적으면 조용히 예전 모양으로 돌아갑니다.
 * "번갈아 간다"는 규칙 자체이므로 규칙으로 둡니다.
 */
export function swingDirection(stepIndex: number): 1 | -1 {
  return stepIndex % 2 === 0 ? 1 : -1
}
...
export const WEAPONS: WeaponDef[] = [
  {
    ...
    combo: [
      /**
       * ── ⏱ **잇는 것이 빠른 무기** — 마무리가 표준 창 안에 들어오게 ────
       *
       * `npm run weapons` 의 시간표가 문제를 그대로 보여 줬습니다:
       *
       *     잡몹의 표준 반격 창 = **0.73초**
       *     롱소드 1타 0.15 · 2타 0.47 · **3타 0.87** ← 창 밖
       *
       * 이 무기의 정체성은 *"3타 콤보의 마무리가 크다"* 입니다 — 27 피해로
       * 한 바퀴(53)의 절반이 거기 있습니다. 그런데 그 마무리가 **가장 자주
       * 오는 창에서는 원리적으로 안 나옵니다.** 그래서 표준 창에서 롱소드가
       * 낼 수 있는 것은 언제나 12+14=26 뿐이었고, 세 무기가 26·26·24 로
       * 평평했습니다. 만능형이 아니라 **아무 데서도 안 이기는 무기**입니다.
       *
       * 니오의 자세, 오공의 봉술, 엘든 링의 무기군이 공통으로 지키는 것이
       * 하나 있습니다 — **무기마다 자기가 가장 잘 쓰는 창이 있다.**
       * 이 게임의 창은 셋이고, 둘은 이미 임자가 있었습니다:
       *
       *     잡몹 무너짐(1.0초) → 대검 (한 방이 크고 강인도를 깎습니다)
       *     보스 무너짐(1.9초) → 쌍단검 (오래 붙어 4타를 반복합니다)
       *     표준 반격창(0.73초) → **비어 있었습니다**
       *
       * 그 자리를 롱소드에게 줍니다. 방법은 피해를 올리는 것이 아니라
       * **잇는 시간을 줄이는 것**입니다(1·2타 후딜 0.20/0.22 → 0.08,
       * 3타 예고 0.22 → 0.12). 마무리는 **0.65초**에 꽂히고, 그 결과
       * 표준 반격 창의 1등이 롱소드가 됐습니다(53 · 대검 26 · 쌍단검 24).
       *
       * ── ⚠️ 아직 **여유가 0.05초뿐**입니다 (벤치의 빨간 줄) ───────────
       * 세 번 재고 세 번 다 문턱(0.08초)에 못 미쳤습니다:
       *
       *     후딜 0.12/0.12 · 예고 0.18 → 마무리 0.72초 (여유 0.017)
       *     후딜 0.08/0.08 · 예고 0.12 → 마무리 0.65초 (여유 0.050)  ← 지금
       *     후딜 0.06/0.05 · 예고 0.12 → 마무리 **0.67초** (여유 0.033)
       *
       * 세 번째가 요점입니다 — **더 줄였는데 오히려 늦어졌습니다.** 타격
       * 시각은 후딜의 단순 합이 아니라 콤보 창·선입력과 얽혀 있어서, 여기서
       * 더 짜내는 것은 값을 만지는 게 아니라 **더듬는 것**입니다.
       *
       * 여유 0.08초를 채우려면 예고까지 쌍단검 수준(0.09)으로 내려야 하는데,
       * 그러면 두 무기의 박자가 사실상 같아집니다 — **이기는 창을 만들려다
       * 무기 하나를 지우는 것**입니다. 그래서 여기서 멈추고 빨간 줄을 남깁니다.
       * 벤치가 두 번 다 그대로 잡았습니다:
       * *"창 경계에 종이 한 장 차이로 걸친 타가 없다"* ❌. 그 자리에 두면
       * 마무리가 들어가느냐 마느냐가 **동전 던지기**가 됩니다. 이 저장소가
       * 계속 적어 온 그대로 — **한 칸 차이의 초록은 기준선이 아니라 운입니다.**
       * 그래서 0.58초까지 밀어 넣어 여유를 0.12초 만들었습니다.
       *
       * ⚠️ 그 대가로 3타 예고가 1타와 같아졌습니다(0.12). "무거운 마무리"의
       *    읽기 단서는 이제 **예고 길이가 아니라 충격**이 냅니다 —
       *    히트스톱 0.11(1타의 두 배) · 트라우마 0.5 · 넉백 4.2 · 부채꼴
       *    150° · 피해 27. 니오·오공의 빠른 무기가 마무리를 *느끼게* 하는
       *    방식이 정확히 이것입니다: 뜸이 아니라 **한 방의 무게**로.
       *
       * ⚠️ 피해·기력·사거리는 **한 숫자도 안 건드렸습니다.** 총 DPS 가
       *    오르는 것은 목적이 아니라 결과입니다. 목적은 *"이 무기가 이기는
       *    창이 하나는 있어야 한다"* 입니다.
       */
      { name: '1타', windup: 0.12, active: 0.08, recovery: 0.08, damage: 12, range: 2.3, arcDeg: 110, staminaCost: 10, hitstop: 0.055, trauma: 0.22, lunge: 1.5, knockback: 1.6 },
      { name: '2타', windup: 0.1, active: 0.08, recovery: 0.08, damage: 14, range: 2.3, arcDeg: 120, staminaCost: 11, hitstop: 0.06, trauma: 0.26, lunge: 1.7, knockback: 1.8 },
      { name: '3타(마무리)', windup: 0.12, active: 0.1, recovery: 0.42, damage: 27, range: 2.7, arcDeg: 150, staminaCost: 17, hitstop: 0.11, trauma: 0.5, lunge: 2.4, knockback: 4.2 },
    ],
  },
  {
    id: 'greatsword',
