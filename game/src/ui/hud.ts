...
export class Hud {
  ...
  setFocus(value: number, max: number): void {
     ...
     *
     * 가득 찬 뒤에 들어오는 집중은 **그대로 버려집니다.** 그런데 화면은
     * 3/3 을 그냥 "다 켜짐"으로만 보여 줬고, 그건 *잘 되고 있다* 로
     * 읽힙니다 — 실제로는 그 순간부터 때리는 족족 사라지는 중인데요.
     * 오공의 봉세가 최대 단계에서 요란해지는 이유가 정확히 이것입니다.
     *
     * ⚠️ **처음엔 벤치의 「태움 21.0 vs 흘림 29.7」을 근거로 적었다가
     *    지웠습니다.** 봇의 강타 규칙이 `가득 찼을 때만 태운다` 라서,
     *    그 흘림은 **경제가 아니라 봇 정책**이 만든 값이었습니다. 신호를
     *    다는 것 자체는 여전히 옳지만(가득 참 = 버리는 중), 근거를 잘못
     *    든 것은 잘못입니다. 봇의 숫자를 게임의 성질로 읽지 않습니다.
     *
